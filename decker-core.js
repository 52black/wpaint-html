// Decker Core JavaScript - Version 1.56
...
clchars=x=>x.normalize("NFC").replace(
	/(\r)|(\t)|([‘’])|([“”])|([^\x20-\x7E\n…ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖØÙÚÛÜÝÞßàáâãäåæçèéêëìíîïðñòóôõöøùúûüýþÿĀāĂăĄąĆćĒēĘęĪīıŁłŃńŌōŐőŒœŚśŠšŪūŰűŸŹźŻżŽžȘșȚțẞ¡¿«»€°\u4e00-\u9fff\u3400-\u4dbf\uf900-\ufaff\u3000-\u303f\uff00-\uffef]$)/gm,
...
dkix=(dict,key)=>dict.k?dict.k.findIndex(x=>match(key,x)):-1, dget=(dict,key)=>dict.v?dict.v[dkix(dict,key)]:undefined
dvix=(dict,val)=>dict.v?dict.v.findIndex(x=>match(val,x)):-1, dkey=(dict,val)=>dict.k?dict.k[dvix(dict,val)]:undefined
dset=(d,k,v)=>{if(!d.k)d.k=[];if(!d.v)d.v=[];const i=d.k.findIndex(x=>match(x,k));if(i<0){d.k.push(k),d.v.push(v)}else{d.v[i]=v};return d}
...
// 简单绘制线：根据 brush 绘制，支持条件擦除
draw_line_simple=(r,brush,pattern,erasePatternRange)=>{
	...
	let dx=abs(r.w-r.x), dy=-abs(r.h-r.y), err=dx+dy, sx=r.x<r.w ?1:-1, sy=r.y<r.h?1:-1;while(1){
		if(brush==0){
			if(inclip(r)){
				if(erasePatternRange){
					// 条件擦除：只清除指定范围内的 pattern
					const currentPat=gpix(r);
					const minPat=erasePatternRange[0], maxPat=erasePatternRange[1];
					if(currentPat>=minPat && currentPat<=maxPat){
						pix(r,pattern); // 在范围内，清除
					}
					// 不在范围内，保持原值（不修改）
				}else{
					pix(r,pattern); // 正常绘制
			...
			for(let b=0;b<8;b++)for(let a=0;a<8;a++){
				const h=rect(r.x+a-3,r.y+b-3);
				if(bsh(brush,a,b)&&inclip(h)){
					if(erasePatternRange){
						// 条件擦除
						const currentPat=gpix(h);
						const minPat=erasePatternRange[0], maxPat=erasePatternRange[1];
						if(currentPat>=minPat && currentPat<=maxPat){
							pix(h,pattern); // 在范围内，清除
						}
						// 不在范围内，保持原值
					}else{
						pix(h,pattern); // 正常绘制
...
draw_line_custom=(r,mask,pattern,erasePatternRange)=>{
	let dx=abs(r.w-r.x), dy=-abs(r.h-r.y), err=dx+dy, sx=r.x<r.w ?1:-1, sy=r.y<r.h?1:-1, ms=mask.size, mc=rint(rdiv(ms,2));while(1){
		for(let b=0;b<ms.y;b++)for(let a=0;a<ms.x;a++){
			const h=rect(r.x+a-mc.x,r.y+b-mc.y), mp=mask.pix[a+b*ms.x];
			if(mp&&inclip(h)){
				if(erasePatternRange){
					// 条件擦除
					const currentPat=gpix(h);
					const minPat=erasePatternRange[0], maxPat=erasePatternRange[1];
					if(currentPat>=minPat && currentPat<=maxPat){
						pix(h,mp==1?pattern: mp==47?1: mp); // 在范围内，清除
					}
					// 不在范围内，保持原值
				}else{
					pix(h,mp==1?pattern: mp==47?1: mp); // 正常绘制
				}
			}
		}
		if(r.x==r.w&&r.y==r.h)break;let e2=err*2; if(e2>=dy)err+=dy,r.x+=sx; if(e2<=dx)err+=dx,r.y+=sy
...
draw_line_function=(r,func,pattern,erasePatternRange)=>{
	...
	let dx=abs(r.w-r.x), dy=-abs(r.h-r.y), err=dx+dy, sx=r.x<r.w ?1:-1, sy=r.y<r.h?1:-1;while(!do_panic){
		...
		if(image_is(v)){
			...
			for(let b=0;b<ms.y;b++)for(let a=0;a<ms.x;a++){
				const h=rect(r.x+a-mc.x,r.y+b-mc.y), mp=v.pix[a+b*ms.x];
				if(mp&&inclip(h)){
					if(erasePatternRange){
						// 条件擦除
						const currentPat=gpix(h);
...
draw_line=(r,brush,pattern,deck,erasePatternRange)=>{
	if(brush>=0&&brush<=23){draw_line_simple(r,brush,pattern,erasePatternRange);return}
	const b=deck.brushes;if(brush<0||brush-24>=b.v.length)return;const f=b.v[brush-24]
	if(image_is(f)){draw_line_custom(r,f,pattern,erasePatternRange)}else if(lion(f)){draw_line_function(r,f,pattern,erasePatternRange)}
}
n_brush=(z,deck)=>{
...
draw_lines=(poly,brush,pattern,deck,erasePatternRange)=>{for(let z=0;z<poly.length-1;z++)draw_line(rpair(poly[z],poly[z+1]),brush,pattern,deck,erasePatternRange)}
poly_bounds=poly=>{
...
// Canvas字体渲染器，用于中文字符渲染
const canvasFontRenderer = {
	canvas: null,
	ctx: null,
	...
	measureWidth(char, fontSize = 12) {
		this.init();
		this.ctx.font = `${fontSize}px ${this.fontFamily}`;
		return this.ctx.measureText(char).width;
	},

	renderChar(char, fontSize = 12, mode = 'direct') {
		this.init();
		this.fontSize = fontSize;
		
		// 检查缓存
		const cacheKey = `${char}_${fontSize}_${mode}`;
		...
		if (mode === 'direct') {
			// console.log('直接渲染')
			// 直接渲染版本：最高性能，不进行任何缩放采样
			const targetSize = fontSize;
			
			this.canvas.width = targetSize;
			this.canvas.height = targetSize*2+1;
			
			// 设置字体
			this.ctx.font = `${targetSize}px ${this.fontFamily}`;
			...
			// 绘制字符
			this.ctx.fillText(char, 0, 1);
			
			// 获取像素数据
			const imageData = this.ctx.getImageData(0, 0, targetSize, targetSize*2);
			const pixels = imageData.data;
			
			// 直接采样alpha通道，无缩放处理
			const bitmap = [];
			for (let i = 0; i < pixels.length; i += 4) {
				bitmap.push(pixels[i + 3]); // 直接使用alpha值
			}
			
			result = { bitmap, width: targetSize, height: targetSize };
		} 
...
// 检查是否需要使用Canvas渲染（非DeckRoman字符，如中文、韩文、泰文等）
function isCanvasChar(char) {
...
// 文本分段器，用于正确处理字素簇（如泰文、Emoji等）
let segmenter;
try {
	segmenter = new Intl.Segmenter(undefined, {granularity: 'grapheme'});
} catch (e) {
	// Fallback for environments without Intl.Segmenter (though unlikely in modern browsers)
	segmenter = { segment: (text) => text.split('').map(char => ({segment: char})) };
}
const segmentText = text => [...segmenter.segment(text)].map(x => x.segment);

// 获取字符宽度（支持中文、泰文等）
function getCharWidth(font, char) {
	if (isCanvasChar(char)) {
		// 泰语字符或组合字符使用实际测量宽度
		if (isThai(char) || char.length > 1) {
			// 增加一点缓冲，避免某些字体渲染紧凑
			return Math.ceil(canvasFontRenderer.measureWidth(char, font_h(font)));
		}
		// 中日韩字符使用字体高度作为宽度（方形字符）
		return font_h(font);
	}
	return font_gw(font, char);
}

draw_char=(pos,font,c,pattern,renderMode='balanced')=>{
	// 如果是中文字符，使用canvas渲染
	if (isCanvasChar(c)) {
		const rendered = canvasFontRenderer.renderChar(c, font_h(font), renderMode);
		for (let a = 0; a < rendered.height; a++) {
			for (let b = 0; b < rendered.width; b++) {
				// 调整阈值以适应新的渲染算法，保留更多细节
				const alpha = rendered.bitmap[b + a * rendered.width];
				if (alpha > 32) { // 降低阈值以显示更多细节笔画
					const h = rect(pos.x + b, pos.y + a);
					if (inclip(h)) pix(h, pattern);
				}
			}
		}
		return;
...
draw_text=(pos,text,font,pattern)=>{
	...
	const cursor=rect(pos.x,pos.y);for(let z=0;z<segments.length;z++){
		const c=segments[z];
		if(c!='\n'){
			draw_char(cursor,font,c,pattern)
			// 为中文字符增加额外间距以避免拥挤
			const charWidth = getCharWidth(font,c)
			const spacing = (isCanvasChar(c) && !isThai(c) && c.length === 1) ? font_sw(font) + 1 : font_sw(font)
			cursor.x += charWidth + spacing
		}else{
			cursor.x=pos.x,cursor.y+=font_h(font)
		}
	}
}
const ELLIPSIS='…'
layout_plaintext=(text,font,align,mx)=>{
	...
	for(let z=0;z<segments.length;z++){
		let a=z,w=segments[z]=='\n'?0:(getCharWidth(font,segments[z])+((isCanvasChar(segments[z]) && !isThai(segments[z]) && segments[z].length === 1) ? fs + 1 : fs))
		if(!ws(segments[z]))while(segments[z+1]&&!ws(segments[z+1]))w+=getCharWidth(font,segments[++z])+((isCanvasChar(segments[z]) && !isThai(segments[z]) && segments[z].length === 1) ? fs + 1 : fs) // find word
		if(cursor.x+w>=mx.x&&cursor.x>0)lnl() // word won't fit this line
		for(let i=a;i<=z;i++){                 // append word to line
			const c=segments[i], spacing = (isCanvasChar(c) && !isThai(c) && c.length === 1) ? fs + 1 : fs, size=rect(c=='\n'?0:getCharWidth(font,c)+spacing,fh)
			...
			// 	const ellipsisSpacing = (isCanvasChar(ELLIPSIS) && !isThai(ELLIPSIS)) ? fs + 1 : fs
			// 	layout[layout.length-1].c=ELLIPSIS,layout[layout.length-1].pos.w=getCharWidth(font,ELLIPSIS)+ellipsisSpacing
			// 	z=segments.length-1;break
			// }
		}
	}
...
layout_richtext=(deck,table,font,align,width)=>{
	...
	for(let chunk=0;chunk<texts.length;chunk++){
		...
		const t=segmentText(ls(texts[chunk]));for(let z=0;z<t.length;z++){
			let a=z, w=t[z]=='\n'?0:(getCharWidth(f,t[z])+((isCanvasChar(t[z]) && !isThai(t[z]) && t[z].length === 1) ? fs + 1 : fs))
			if(!ws(t[z]))while(z+1<t.length&&!ws(t[z+1]))w+=getCharWidth(f,t[++z])+((isCanvasChar(t[z]) && !isThai(t[z]) && t[z].length === 1) ? fs + 1 : fs)
			if(cursor.x+w>=width&&cursor.x>0)lnl()
			for(let i=a;i<=z;i++){
				const c=t[i], spacing = (isCanvasChar(c) && !isThai(c) && c.length === 1) ? fs + 1 : fs, size=rect(c=='\n'?0:getCharWidth(f,c)+spacing,fh)
...
image_read=x=>{
	...
	// console.log('image_read: 格式='+f+', 尺寸='+w+'x'+h+', 数据长度='+data.length)
	...
	// console.log('image_read: 实际解码后尺寸='+r.size.x+'x'+r.size.y)
	return r
}
image_write=x=>{
	...
	// console.log('image_write: 输入尺寸='+s.x+'x'+s.y+', 格式='+f+', 输出数据长度='+(f=='2'?l.length:t.length))
...
patterns_read=x=>{
	...
	// console.log('patterns_read: x.patterns存在?', !!x.patterns)
	// console.log('patterns_read: patterns数据长度:', x.patterns ? ls(x.patterns).length : 0)
	let i=image_read(x.patterns?ls(x.patterns):DEFAULT_PATTERNS)
	// console.log('patterns_read: 读取到的图像尺寸:', i.size.x, 'x', i.size.y)
	if(i.size.x!=8||i.size.y!=224+6){
		// console.log('patterns_read: 图像尺寸不对，重置为默认值！')
		i=image_resize(i,rect(8,224+6));
		for(let z=0;z<16;z++)pal_col_set(i.pix,z,DEFAULT_COLORS[z])
	}else{
		// console.log('patterns_read: 图像尺寸正确，读取的颜色值:', Array.from({length:16}, (_,z)=>pal_col_get(i.pix,z).toString(16)))
	}
	ri.pal=i
...
patterns_write=x=>{
	...
	// console.log('patterns_write: 检测到颜色修改:', c)
	// console.log('patterns_write: 当前颜色值:', Array.from({length:16}, (_,i)=>pal_col_get(p,i).toString(16)))
	// console.log('patterns_write: 默认颜色值:', DEFAULT_COLORS.map(c=>(0xFFFFFF&c).toString(16)))
	// 修复：总是包含颜色数据（230行），避免读取时被重置
	const result = image_write(image_resize(image_copy(x.pal),rect(8,230)))
	// console.log('patterns_write: 输出长度:', result.length, '强制包含颜色数据')
	return result
}
...
font_textsize=(f,t)=>{
	...
	for(let z=0;t[z];z++){
		if(t[z]!='\n'){
			// 为中文字符增加额外间距以避免拥挤
			const charWidth = getCharWidth(f,t[z])
			const spacing = (isCanvasChar(t[z]) && !isThai(t[z])) ? font_sw(f) + 1 : font_sw(f)
			cursor.x += charWidth + spacing
			size.x = max(size.x,cursor.x)
		}else{
			cursor.x=0,size.y+=font_h(f)
		}
	}
	return size
}
...
field_read=(x,card)=>{
	const ri=lmi((self,i,x)=>{
		...
		if(x){
			...
			if(ikey(i,'colorpicker'))return lmn(ivalue(self,ls(i),0))
...
field_write=x=>{
	...
	if(x.colorpicker!=undefined)dset(r,lms('colorpicker'),lmn(x.colorpicker))
...
canvas_read=(x,card)=>{
	...
	const ri=lmi((self,i,x)=>{
		...
		if(x){
			...
			if(ikey(i,'line'     ))if(ikey(i,'line'     ))return lmnat(z=>{
				canvas_pick(self);
				let eraseRange=null;
				// 检查最后一个参数是否是 eraseRange [min, max]
				// 策略：如果参数数量>=3，且最后一个参数是长度为2的列表且都是数字，
				// 且最后一个参数的值在合理范围内（0-255），则认为是范围参数
				if(z.length>=3){
					const last=z[z.length-1];
					// console.log('检查最后一个参数:', lil(last), count(last), lin(last.v?last.v[0]:NIL), lin(last.v?last.v[1]:NIL));
					// 检查是否是 [min, max] 格式：列表，长度为2，且都是数字，且值在0-255范围内
					if(lil(last) && count(last)==2 && lin(last.v[0]) && lin(last.v[1])){
						const minVal=ln(last.v[0]), maxVal=ln(last.v[1]);
						// console.log('  值检查:', minVal, maxVal, '范围:', minVal>=0 && minVal<=255, maxVal>=0 && maxVal<=255, minVal<=maxVal);
						// 如果值在0-255范围内，且min<=max，则认为是范围参数
						if(minVal>=0 && minVal<=255 && maxVal>=0 && maxVal<=255 && minVal<=maxVal){
							eraseRange=[minVal,maxVal];
							// 移除最后一个参数，剩下的作为点列表
							z=z.slice(0,z.length-1);
							// console.log('✓ 条件擦除范围:', eraseRange, '剩余参数数量:', z.length);
						}
					}
				}
				const poly=unpack_poly(z);
				// console.log('线条点列表:', poly.map(p=>`(${p.x},${p.y})`).join(' -> '), '范围:', eraseRange);
				draw_lines(poly,frame.brush,frame.pattern,self.card.deck,eraseRange);
				return self;
			})
...
interface_widget=(self,i,x)=>{
	...
	if(x){
		...
		// 处理自定义属性（如 colorpicker）的写入
		if(!ikey(i,'name') && !ikey(i,'index') && !ikey(i,'font') && !ikey(i,'script') && 
		   !ikey(i,'locked') && !ikey(i,'animated') && !ikey(i,'volatile') && 
		   !ikey(i,'size') && !ikey(i,'pos') && !ikey(i,'show')){
			// 存储到 self 对象中
			self[ls(i)] = lb(x) ? lb(x) : (lin(x) ? ln(x) : (lis(x) ? ls(x) : x))
			return x
		}
	}else{
		if(ikey(i,'name'    ))return lms(self.name)
		...
		// 处理自定义属性（如 colorpicker），从 self 对象或数据中读取
		if(self.hasOwnProperty && self.hasOwnProperty(ls(i))){
			const val = self[ls(i)]
			return typeof val === 'boolean' ? lmn(val ? 1 : 0) : 
			       typeof val === 'number' ? lmn(val) :
			       typeof val === 'string' ? lms(val) : val
		}
		// 尝试从 b 数据中读取
		if(self.b){
			const val = dget(self.b, i)
			if(!linil(val)) return val
		}
	}return x?x:NIL
}
widget_read=(x,card)=>{
	...
	init_field(ri,'colorpicker',x) // 初始化 colorpicker 属性
	return ri
}
widget_write=x=>{
	const r=lmd()
...
deck_read=x=>{
	...
	// 修复：跳过<body>和<script>之间的所有内容
	if(match('<body>')){
		// 查找 <script language="decker">
		const scriptStart = x.indexOf('<script language="decker">', i);
		if(scriptStart !== -1) {
			i = scriptStart + '<script language="decker">'.length;
			console.log('deck_read: 找到script标签，跳转到位置', i);
		}
	}
	match('<body><script language="decker">');while(!end()){
		if(x[i]=='\n')i++
		...
		else if(match('{autoswitch_config}\n')){console.log('Found autoswitch_config section');m=8}
		else if(m==8&&match('{end}\n')){console.log('End of autoswitch_config section');m=1}
			...
			if(m==1){
				deck[k]=v
				// if(k==='patterns')console.log('deck_read: 解析到patterns字段，值长度:', v ? ls(v).length : 0)
			}
			...
			if(m==8){
				console.log('Reading autoswitch config:', k, '=', v)
				deck['autoswitch_'+k]=v
			}
		}
	}
	...
	// 将自动切换配置添加到ri对象
	if (deck.hasOwnProperty('autoswitch_uniformInterval')) {
		ri['autoswitch_uniformInterval'] = deck['autoswitch_uniformInterval']
		console.log('Added autoswitch_uniformInterval to ri:', deck['autoswitch_uniformInterval'])
	}
	if (deck.hasOwnProperty('autoswitch_intervals')) {
		ri['autoswitch_intervals'] = deck['autoswitch_intervals']
		console.log('Added autoswitch_intervals to ri:', deck['autoswitch_intervals'])
	}
	if (deck.hasOwnProperty('autoswitch_startCard')) {
		ri['autoswitch_startCard'] = deck['autoswitch_startCard']
		console.log('Added autoswitch_startCard to ri:', deck['autoswitch_startCard'])
	}
	if (deck.hasOwnProperty('autoswitch_endCard')) {
		ri['autoswitch_endCard'] = deck['autoswitch_endCard']
		console.log('Added autoswitch_endCard to ri:', deck['autoswitch_endCard'])
	}
	if (deck.hasOwnProperty('autoswitch_isEnabled')) {
		ri['autoswitch_isEnabled'] = deck['autoswitch_isEnabled']
		console.log('Added autoswitch_isEnabled to ri:', deck['autoswitch_isEnabled'])
	}
...
deck_write=(x,html)=>{
	...
	//console.log('deck_write: pp长度:', pp.length, 'DEFAULT_PATTERNS长度:', DEFAULT_PATTERNS.length)
	//console.log('deck_write: pp != DEFAULT_PATTERNS?', pp!=DEFAULT_PATTERNS)
	...
	// 跳过值为 'undefined' 的脚本
	write_line(x,'script'    ,x=>x&&x.length&&x!='undefined'        ,x=>script_ref(null,lms(x)))
	write_line(x,'name'      ,x=>x.length                           ,lms                       )
	write_line(x,'author'    ,x=>x.length                           ,lms                       )
	const should_write_patterns = pp!=DEFAULT_PATTERNS
	// console.log('deck_write: should_write_patterns?', should_write_patterns)
	write_line(x,'patterns'  ,x=>pp!=DEFAULT_PATTERNS               ,x=>lms(pp)                )
	// console.log('deck_write: patterns字段已写入，检查r字符串是否包含patterns:', r.includes('patterns:'))
	...
	deck.cards.v.map(c=>{
		...
		// 跳过值为 'undefined' 的组件脚本
		wids.v.map(wid=>{const k=lms('script'),v=dget(wid,k);if(v&&ls(v)!='undefined')dset(wid,k,script_ref(base,v))})
		write_dict('{widgets}\n',wids,x=>x)
		write_scripts()
	})
	...
	deck.contraptions.v.map(def=>{
		...
		write_key(data,'version'    ,x=>x       ,x=>x)
		write_key(data,'image'      ,x=>x       ,x=>x)
		// 跳过值为 'undefined' 的原型脚本
		write_key(data,'script'     ,x=>count(x)&&ls(x)!='undefined',x=>script_ref(base,x,'p'))
		write_key(data,'script'     ,x=>count(x)&&ls(x)!='undefined',x=>script_ref(base,x,'p'))
	...
	// 添加自动切换配置到deck文件末尾
	if (typeof autoSwitchState !== 'undefined' && autoSwitchState.intervals && autoSwitchState.intervals.length > 0) {
		console.log('Saving autoswitch config:', autoSwitchState)
		r += '\n{autoswitch_config}\n'
		r += `uniformInterval:${autoSwitchState.uniformInterval}\n`
		r += `intervals:${JSON.stringify(autoSwitchState.intervals)}\n`
		r += `startCard:${autoSwitchState.startCard || 0}\n`
		r += `endCard:${autoSwitchState.endCard || (x.cards ? x.cards.v.length - 1 : 0)}\n`
		r += `isEnabled:${autoSwitchState.isEnabled ? 1 : 0}\n`
		r += '{end}\n'
	}
	return r+'\n'+(html?'<\/script>\nRuntime stub is NYI.':'')
}
...
if(typeof window!=='undefined'&&!window.ext)window.ext={}
const ext=(typeof window!=='undefined')?window.ext:{}
const ext_constants={}
constants=env=>{
...
primitives=(env,deck)=>{
	...
	typeof n_log               !=='undefined'&&env.local('log'     ,lmnat(n_log   ))
	typeof n_quickColor        !=='undefined'&&env.local('quickColor',lmnat(n_quickColor))
	typeof n_showLanguagePicker!=='undefined'&&env.local('showLanguagePicker',lmnat(n_showLanguagePicker))
	typeof n_isHideNav         !=='undefined'&&env.local('isHideNav' ,lmnat(n_isHideNav))
	typeof n_t_card            !=='undefined'&&env.local('t_card'     ,lmnat(n_t_card))
	typeof n_log2              !=='undefined'&&env.local('log2'    ,lmnat(n_log2  ))
	typeof n_translate         !=='undefined'&&env.local('t'       ,lmnat(([key,...params])=>n_translate([key,...params],deck)))
	env.local('current_lang',lmnat(()=>lms(typeof i18n!=='undefined'&&i18n&&i18n.currentLang?i18n.currentLang:'en')))
	typeof n_refresh           !=='undefined'&&env.local('refresh',lmnat(n_refresh))
	typeof n_get_last_card     !=='undefined'&&env.local('get_last_card',lmnat(n_get_last_card))
	typeof n_debugger          !=='undefined'&&env.local('debugger'     ,lmnat(n_debugger   ))
...
// 自动切换卡片功能的Lil接口
n_autoswitch_start = (args, env) => {
	const startCard = args.length > 0 ? ln(args[0]) : 0
	const endCard = args.length > 1 ? ln(args[1]) : Math.max(0, getCardCount() - 1)
	const interval = args.length > 2 ? ln(args[2]) : 3000
	
	autoSwitchState.startCard = startCard
	autoSwitchState.endCard = endCard
	autoSwitchState.uniformInterval = interval
	autoSwitchState.useUniformInterval = true
	
	startAutoSwitch()
	return ONE
}

n_autoswitch_stop = (args, env) => {
	stopAutoSwitch()
	return ONE
}

n_autoswitch_config = (args, env) => {
	if (args.length === 0) {
		// 返回当前配置
		return lmd(
			['enabled', 'startCard', 'endCard', 'uniformInterval', 'useUniformInterval'].map(lms),
			[lmn(autoSwitchState.enabled ? 1 : 0), lmn(autoSwitchState.startCard), 
			 lmn(autoSwitchState.endCard), lmn(autoSwitchState.uniformInterval), 
			 lmn(autoSwitchState.useUniformInterval ? 1 : 0)]
		)
	}
	...
	const config = args[0]
	if (lid(config)) {
		const enabled = dget(config, lms('enabled'))
		const startCard = dget(config, lms('startCard'))
		const endCard = dget(config, lms('endCard'))
		const uniformInterval = dget(config, lms('uniformInterval'))
		const useUniformInterval = dget(config, lms('useUniformInterval'))
		
		if (startCard) autoSwitchState.startCard = ln(startCard)
		if (endCard) autoSwitchState.endCard = ln(endCard)
		if (uniformInterval) autoSwitchState.uniformInterval = ln(uniformInterval)
		if (useUniformInterval) autoSwitchState.useUniformInterval = ln(useUniformInterval) > 0
		
		if (enabled && ln(enabled) > 0 && !autoSwitchState.enabled) {
			startAutoSwitch()
		} else if (enabled && ln(enabled) === 0 && autoSwitchState.enabled) {
			stopAutoSwitch()
		}
	}
...
// Decker
//   if(window.DECK_DATA) {
//       document.write('<script language="decker">' + window.DECK_DATA + '<\/script>');
//   }
...
const _t = (key, params) => {
  if (typeof i18n !== 'undefined' && i18n && typeof i18n.t === 'function') {
    try {
      // 确保 i18n 已初始化
      if (!i18n.currentLang && typeof i18n.init === 'function') {
        i18n.init();
      }
      const result = i18n.t(key, params);
      // 如果返回的是 key 本身，说明翻译不存在，返回 key
      return result || key;
    } catch (e) {
      console.warn('i18n.t error:', e, key);
      return key;
    }
  }
  return key;
}
const _tlms = (key, params) => lms(_t(key, params))

// 文件保存状态跟踪
window.isSaved = true // 标记文件是否已保存

// Canvas录制相关变量
let canvasRecorder = null
let recordedChunks = []
let isRecording = false
let recordingFrameRate = 30 // 提高到30帧以获得更好质量
...
open_url=x=>{
	// 检查是否在Android环境中
	if(typeof AndroidFileInterface !== 'undefined' && AndroidFileInterface.openUrl) {
		AndroidFileInterface.openUrl(x)
	} else if(typeof cordova !== 'undefined' && cordova.InAppBrowser) {
		// Cordova环境（包括iOS）
		cordova.InAppBrowser.open(x, '_system')
	} else if(typeof webkit !== 'undefined' && webkit.messageHandlers && webkit.messageHandlers.openUrl) {
		// Xcode环境（iOS WKWebView）
		webkit.messageHandlers.openUrl.postMessage(x)
	} else {
		// 浏览器环境的回退方案
...
save_text=(n,x)=>{
	// 检查是否在Android环境中
	if(typeof AndroidFileInterface !== 'undefined' && AndroidFileInterface.openSaveFileDialog) {
		AndroidFileInterface.openSaveFileDialog(n, x)
		...
		window.resolveLocalFileSystemURL(cordova.file.documentsDirectory, function(dirEntry) {
			dirEntry.getFile(n, {create: true, exclusive: false}, function(fileEntry) {
				fileEntry.createWriter(function(fileWriter) {
					fileWriter.write(new Blob([x], {type: 'text/plain'}))
				})
			})
		})
	} else if(typeof webkit !== 'undefined' && webkit.messageHandlers && webkit.messageHandlers.saveFile) {
		// Xcode环境（iOS WKWebView）
		webkit.messageHandlers.saveFile.postMessage({filename: n, content: x, type: 'text'})
	} else {
		// 浏览器环境的回退方案
...
save_bin=(n,x)=>{
	...
	if(typeof AndroidFileInterface !== 'undefined' && AndroidFileInterface.openSaveFileDialog) {
		// 将二进制数据转换为Base64字符串
		const base64 = btoa(String.fromCharCode.apply(null, Uint8Array.from(x)))
		AndroidFileInterface.openSaveFileDialog(n, base64)
	} else if(typeof cordova !== 'undefined' && cordova.file) {
		// Cordova环境
		window.resolveLocalFileSystemURL(cordova.file.documentsDirectory, function(dirEntry) {
			dirEntry.getFile(n, {create: true, exclusive: false}, function(fileEntry) {
				fileEntry.createWriter(function(fileWriter) {
					fileWriter.write(new Blob([Uint8Array.from(x)]))
				})
			})
		})
	} else if(typeof webkit !== 'undefined' && webkit.messageHandlers && webkit.messageHandlers.fileSaver) {
		// Xcode环境（iOS WKWebView）
		// 将二进制数据转换为Base64字符串
//		const base64 = btoa(String.fromCharCode.apply(null, Uint8Array.from(x)))
//		webkit.messageHandlers.saveFile.postMessage({filename: n, content: base64, type: 'binary'})
        
        // wigglypaint.io 这里用fileSaver的api来处理.
        const dataArray = Uint8Array.from(x);
        let binaryString = '';
        for (let i = 0; i < dataArray.length; i++) {
            binaryString += String.fromCharCode(dataArray[i]);
        }
        const base64Data = btoa(binaryString);
        
        window.webkit.messageHandlers.fileSaver.postMessage({
            filename: n,
...
save_deck=async(n,x)=>{
	let d=deck_write(x)
// 	if(/\.html$/i.test(n)){
// 		// const scriptElement = q('script[language="decker"]')
// 		// if(scriptElement) {
// 		// 	scriptElement.innerHTML='\n'+d
// 		// 	d=`<meta charset="UTF-8"><body>${q('body').innerHTML}</body>`
// 		// } else {
// 			// 如果没有找到script元素，创建一个包含deck数据的HTML
// 			try {
// 				// 使用window.coreName获取脚本URL
// 				const coreUrl = window.coreName || 'decker-external-core.js';
				
// 				// 检查缓存
// 				if (!window.cachedCoreScript) {
// 					// 获取脚本内容并缓存
// 					const response = await fetch(coreUrl);
// 					window.cachedCoreScript = await response.text();
// 				}
				
// 				// 使用字符串拼接而不是模板字符串来避免语法冲突
// 				// 对脚本内容中的</和-->进行转义，防止提前关闭script标签或HTML注释
// 				const scriptContent = window.cachedCoreScript.replace(/<\//g, '<\\/').replace(/-->/g, '-- >');
				
// 				// 创建包含外部脚本和deck数据的HTML
// 				d='<meta charset="UTF-8"><body>\n' 
...
// 				+ '\n<\/script><script>\n' + scriptContent + '\n<\/script></body>'
// 			} catch (error) {
// 				console.error('Failed to fetch external script:', error);
// 				// 如果获取失败，回退到原来的方式
// 				// d=`<meta charset="UTF-8"><body><script language="decker">\n${d}\n<\/script><\/body>`
// 			}
// 		// }
// 	}
//先把保存成html禁用了，主要是源代码保护
	save_text(n,d)
	// 保存成功后标记为已保存状态
	isSaved = true
}
...
writegif=(frames,delays,transparent=true)=>{
	...
	const draw_pattern    =(pix,x,y)=>pix<2?(pix?1:0): pix>31?(pix==32?0:1): pal_pat(pal,pix,x,y)&1
	const draw_color_trans=(pix,x,y)=>pix==ANTS?anim_ants(x,y): pix==0?(transparent?16:0): pix>47?0: pix>31?pix-32: draw_pattern(pix,x,y)?15:0
	...
	for(let z=0;z<frames.length;z++){
		...
		if(transparent){
			...
			b(2),s(delays[z]),b(0)                // dispose to background (restore to bg color), 100ths of a second delay, no transparency
		}
		b(0)                                      // end GCE
...
draw_modalbox=s=>{
	const menu=(typeof window!='undefined'&&window.HIDE_NAV)?0:16
	const center=rcenter(rect(0,menu,frame.size.x,frame.size.y-menu),s)
	
	// Calculate allowed offset range (allow 50% of window to go outside screen)
	// Window position range: [-s.x*0.5, frame.size.x - s.x*0.5]
	// Since r = center + offset, we get: offset = target_position - center
	const min_offset_x=-s.x*0.5-center.x
	const max_offset_x=frame.size.x-s.x*0.5-center.x
	const min_offset_y=menu-s.y*0.5-center.y
	const max_offset_y=frame.size.y-s.y*0.5-center.y
	
	// Title bar for dragging (top 20 pixels, relative to center position)
	const titlebar_y=center.y+ms.drag_offset.y-5
	const titlebar=rect(center.x+ms.drag_offset.x,titlebar_y,s.x,20)
	
	// Handle dragging
	if(ev.md&&over(titlebar)&&!ms.dragging){
		ms.dragging=1
		ms.drag_start.x=ev.pos.x-ms.drag_offset.x
		ms.drag_start.y=ev.pos.y-ms.drag_offset.y
	}
	if(ev.drag&&ms.dragging){
		// Calculate new offset and clamp immediately
		ms.drag_offset.x=clamp(min_offset_x,ev.pos.x-ms.drag_start.x,max_offset_x)
		ms.drag_offset.y=clamp(min_offset_y,ev.pos.y-ms.drag_start.y,max_offset_y)
		uicursor=cursor.drag
	}
	if(ev.mu)ms.dragging=0
	
	// Show grab cursor on hover
	if(over(titlebar)&&!ms.dragging)uicursor='grab'
	
	// Apply clamped drag offset to final position
	const r=rect(center.x+ms.drag_offset.x,center.y+ms.drag_offset.y,s.x,s.y)
	
	// Set clipping to prevent wraparound when window goes offscreen
	const screen_bounds=rect(0,menu,frame.size.x,frame.size.y-menu)
	const old_clip=frame.clip
	frame.clip=rclip(screen_bounds,old_clip)
	
	const o=inset(r,-5)
	draw_rect(inset(o,-5),32),draw_box(inset(o,-5),0,1),draw_box(inset(o,-2),0,1),draw_box(inset(o,-1),0,1)
	
	// Restore clip for content drawing
	frame.clip=old_clip
	
	// Return clipped rect to prevent wraparound artifacts in content
	return rclip(r,screen_bounds)
}
...
draw_text_fit=(r,text,font,pattern)=>{
	...
	// Use segmentText from lil.js if available, otherwise fallback
	const segments = (typeof segmentText === 'function') ? segmentText(text) : text.split('');
	
	for(let z=0;z<segments.length&&(y+fh)<=r.h;z++){
		const c=segments[z]
		const cw = getCharWidth(font,c)
		const fs = font_sw(font)
		//原先是+1，改成了+0，因为文本框暂时不需要间隔
		const spacing = (isCanvasChar(c) && !isThai(c) && c.length === 1) ? fs + 0 : fs
		
		if(c=='\n'){
			x=0,y+=fh
		}
		else if(x+cw>=r.w && x>0){
			x=0; y+=fh; z--;
		}else{
			glyph_push(rect(x,y),c),
			x+=cw+spacing
		}
	}
	let yo=ceil((r.h-(y+fh))/2.0);
	glyphs.map(g=>{
		g.pos.x+=r.x,
		g.pos.y+=yo+r.y,
		draw_char(g.pos,font,g.c,pattern)
	})
}
...
unpack_field=x=>({
	...
	colorpicker:lb(ifield(x,'colorpicker')),
	border   :lb(ifield(x,'border'   )),
	style    :ls(ifield(x,'style'    )),
	locked   :lb(ifield(x,'locked'   )),
...
mark_saved=_=>{isSaved=false;}

mark_dirty=_=>{dirty=1;_ || mark_saved()}
con_set=x=>{
...
tracking=_=>{
	...
	if(card_is(c)){
		...
		if(ev.dir=='right')n_go([_tlms('next_card')],deck)
	}
}
...
modal_state=_=>({ // modal state
	...
	drag_offset:rect(), dragging:0, drag_start:rect(),
})
modal_state_clone=x=>{const r=Object.assign({},x);r.cell=rcopy(r.cell);r.drag_offset=rcopy(r.drag_offset);r.drag_start=rcopy(r.drag_start);return r}
...
menus_hidden=_=>((typeof window!='undefined'&&window.HIDE_NAV)||(uimode=='draw'&&ev.hidemenu&&ms.type==null))
...
widget_slider=(target,x)=>{
	...
	if(in_layer()&&over(frame.clip)&&ev.scroll)x.value+=x.step*ev.scroll
	if(target){
		x.value=slider_normalize(target,x.value)
	}else{
		const step=(x.step==undefined)?1:x.step
		if(step>0){
			x.value=x.min+Math.round((x.value-x.min)/step)*step
		}
		x.value=clamp(x.min,x.value,x.max)
	}
...
grid_keys=(code,shift)=>{
	...
	if(!wid.g.locked&&(code=='Backspace'||code==_t('delete')))grid_deleterow()
...
field_exit=_=>{field_change(),kc.on=0;wid.infield=0,wid.fv=null,wid.ft=null,wid.cursor=rect(),wid.field_dirty=0,wid.change_timer=0;if(hiddenInput){hiddenInput.style.display='none';hiddenInput.blur();isInputMethodActive=false}}
widget_field=(target,x,value)=>{
	...
	if(!x.locked&&!l&&dover(bi)&&(ev.md||ev.mu||ev.drag)){
		// 检查是否是颜色选择器字段
		const isColorPicker = target && lb(ifield(target, 'colorpicker'))
		if(isColorPicker && ev.mu && !ev.drag){
			// 如果是颜色选择器字段且是点击事件（不是拖拽），打开调色板
			// 先设置字段状态以便 show_color_picker 可以读取当前值
			wid.active=wid.count
			wid.f=x
			wid.fv=value
			wid.ft=target
			// 调用调色板
	...
	if(sel){if(wid.gv)grid_exit();wid.infield=1,wid.f=x,wid.fv=value,wid.ft=target,keycaps_enter();activateInputMethod()}
	// render
...
n_quickColor=_=>{
	if(typeof ext !== 'undefined' && ext.colorPicker && typeof ext.colorPicker.show === 'function'){
		ext.colorPicker.show();
	} else if(typeof show_color_picker_for_pattern === 'function'){
		show_color_picker_for_pattern(dr.pattern);
	}
	return lmn(0);
}
n_showLanguagePicker=args=>{
	if(typeof ext !== 'undefined' && ext.languagePicker && typeof ext.languagePicker.show === 'function'){
		const callback = args[0]
		ext.languagePicker.show(()=>{
			if(callback && (lion(callback))){
				const p=lmblk();blk_lit(p,callback),blk_lit(p,lml([])),blk_op(p,op.CALL)
				const e=lmenv();pushstate(e),issue(e,p);while(running())runop();popstate()
			}
		})
	}
	return lmn(0)
}
n_isHideNav=_=>lmn((typeof window!='undefined'&&window.HIDE_NAV)?1:0)

n_log2=(a)=>{
	const show2=(x,depth=0)=>{
		if(linil(x))return 'nil'
		if(lin(x))return ln(x).toString()
		if(lis(x)){
			const s=ls(x)
		...
		if(lil(x)){
			const c=count(x)
			if(c==0)return '()'
			return `(${x.v.map(v=>show2(v,depth+1)).join(',')})`
		}
		if(lid(x)){
			const c=count(x)
			if(c==0)return '{}'
			return `{${x.k.map((k,i)=>`${show2(k,depth+1)}:${show2(x.v[i],depth+1)}`).join(',')}}`
		}
		if(lit(x))return `(table: ${tab_cols(x).length} cols, ${tab_rowcount(x)} rows)`
		if(lion(x))return `on ${x.n}...`
		if(linat(x))return 'native'
		if(lii(x))return `<${x.n}>`
		return show(x)
	}
	if(a.length<2){
		console.log(show2(a[0]||NIL))
	}else{
		console.log(a.map(show2).join(' '))
	}
	return a[0]||NIL
}
n_translate=([key,...params],deck)=>{
	// 调用 _t 函数获取翻译，避免重复代码
	if(!key)return lms('')
	const keyStr=ls(key)
	// 如果有参数，转换为对象格式
	const paramsObj=params.length>0?Object.fromEntries(params.map((p,i)=>[i,ls(p)])):undefined
	const translated=_t(keyStr,paramsObj)
	return lms(translated)
}
n_refresh=()=>{
	setmode('object');setmode('interact')
}
n_get_last_card=()=>{
	let h=deck.history

	for(let i=h.length-2;i>=0;i--){
		if(!isNaN(+h[i]))return deck.cards.v[h[i]]
	}
}
n_debugger=()=>{
	debugger;
...
const SFX_VOLUME_MAX=300
let sfx_volume_percent=100
try{
	const stored=localStorage.getItem('wpaint_sfx_volume_percent')
	if(stored!=null){
		const n=Number.parseInt(stored,10)
		if(Number.isFinite(n))sfx_volume_percent=Math.max(SFX_VOLUME_MIN,Math.min(SFX_VOLUME_MAX,n))
	}
}catch(e){}
...
n_play=([x,hint])=>{
	const prepare=sfx=>{
		...
		const vol=MASTER_VOLUME*(sfx_volume_percent/100)
		for(let z=0;z<sfx.data.length;z++)for(let b=0;b<8;b++)dest[z*8+b]=vol*byte_to_sample(sfx.data[z])
...
modal_enter=type=>{
	...
	// Reset drag state for new modal
	ms.drag_offset=rect(),ms.dragging=0,ms.drag_start=rect()
	...
	if(type=='deck_props'  ){
		ms.name=fieldstr(ifield(deck     ,'name'))
		ms.text=fieldstr(ifield(deck     ,'author'))
		const size = deck.size || rect(512,342)
		ms.form0=fieldstr(lms(size.x+''))
		ms.form1=fieldstr(lms(size.y+''))
	}
	if(type=='resize_deck'){
		const size = deck.size || rect(512,342)
		ms.form0=fieldstr(lms(size.x+''))
		ms.form1=fieldstr(lms(size.y+''))
		ms.form2=fieldstr(lms('1')) // 靠边选项，0表示未勾选，1表示勾选
		// 保存当前deck数据到临时变量
		ms.temp_deck_data = deck_write(deck)
	}
	...
	if(type=='autoswitch_settings'){
		...
		// 确保默认值正确
		if(autoSwitchState.startCard === 0) autoSwitchState.startCard = 1
		if(autoSwitchState.endCard === 0) autoSwitchState.endCard = Math.max(1, getCardCount() - 5)
		if(autoSwitchState.uniformInterval === 3000) autoSwitchState.uniformInterval = 100
		// 初始化多选状态
		if(autoSwitchState.multiSelect === undefined) autoSwitchState.multiSelect = false
		if(!autoSwitchState.selectedCards) autoSwitchState.selectedCards = []
		
		// 初始化间隔时间列表
		const cardCount = Math.max(1, autoSwitchState.endCard - autoSwitchState.startCard + 1)
		while(autoSwitchState.intervals.length < cardCount) {
			autoSwitchState.intervals.push(autoSwitchState.uniformInterval)
		}
		
		// 获取当前卡片的间隔，而不是使用统一间隔
		const currentCard = ifield(deck, 'card')
		const currentCardIndex = ln(ifield(currentCard, 'index'))
		let currentInterval = autoSwitchState.uniformInterval
		if(currentCardIndex >= autoSwitchState.startCard && currentCardIndex <= autoSwitchState.endCard) {
			const intervalIndex = currentCardIndex - autoSwitchState.startCard
			currentInterval = autoSwitchState.intervals[intervalIndex] || autoSwitchState.uniformInterval
		}
		
		ms.form0=fieldstr(lms(autoSwitchState.startCard+''))  // 起始卡片
		ms.form1=fieldstr(lms(autoSwitchState.endCard+''))    // 结束卡片
		ms.form2=fieldstr(lms(currentInterval+'')) // 当前卡片的间隔时间
		ms.text=fieldstr(lms(autoSwitchState.useUniformInterval?'1':'0')) // 是否使用统一间隔
		
		// 创建卡片列表数据
		const cardData = []
		for(let i = autoSwitchState.startCard; i <= autoSwitchState.endCard; i++) {
			const cardIndex = i - autoSwitchState.startCard
			cardData.push({
				index: i,
				name: `卡片 ${i}`,
				interval: autoSwitchState.intervals[cardIndex] || autoSwitchState.uniformInterval
			})
		}
		ms.grid = gridtab(lt(cardData.map(card => [lms(card.name), lms(card.interval + ' ms')])))
	}
	...
	if(type=='canvas_props'){
		const canvas = ob.sel[0]
		const size = getpair(ifield(canvas,'size'))
		ms.name=fieldstr(ifield(canvas,'name'))
		ms.text=fieldstr(ifield(canvas,'scale'))
		ms.form0=fieldstr(lms(size.x.toString()))
		ms.form1=fieldstr(lms(size.y.toString()))
	}
	...
	if(type=='confirm_new'   )ms.type=type='confirm',ms.subtype='confirm_new'
	if(type=='confirm_script')ms.type=type='confirm',ms.subtype='confirm_script'
	if(type=='multiscript'   )ms.type=type='confirm',ms.subtype='multiscript'
	if(type=='confirm_transparent_bg')ms.type=type='confirm',ms.subtype='confirm_transparent_bg'
	if(type=='save_deck'     )ms.type=type='save',ms.text=fieldstr(dname(deck.name,'.deck')),ms.desc=_t('save_deck_desc')
	if(type=='save_locked'   )ms.type=type='save',ms.text=fieldstr(dname(deck.name,'.html')),ms.desc=_t('save_locked_desc')
	if(type=='export_script' )ms.type=type='save',ms.text=fieldstr(lms('script.lil'  )),ms.desc=_t('export_script_desc')
	if(type=='export_image'  )ms.type=type='save',ms.text=fieldstr(lms('image.gif'   )),ms.desc=_t('export_image_desc')
	if(type=='save_lil'      )ms.type=type='save',ms.text=fieldstr(lms('untitled.txt')),ms.desc=_t('save_lil_desc')
...
modal_exit=value=>{
	...
	if(ms.subtype=='confirm_transparent_bg'){
		// 根据用户选择生成 GIF（value=1 表示透明背景，value=0 表示黑色背景）
		// 使用与 getGifData 相同的 writegif 核心逻辑
		if(ms.pending_gif_frames&&ms.pending_gif_delays){
			try{
				const filename='all_wiggler.gif'
				const transparent=value==1
				// 使用与 getGifData 相同的 writegif 调用方式
				const gifData=writegif(ms.pending_gif_frames,ms.pending_gif_delays,transparent)
				save_bin(filename,gifData)
			}catch(e){
				console.error('Error saving all wiggler GIF:',e)
			}
			// 清理临时数据
			ms.pending_gif_frames=null
			ms.pending_gif_delays=null
		}
	}
...
modals=_=>{
	...
	if(ms.type=='about'){
		const b=draw_modalbox(rect(200,100))
		if(ui_button(rect(b.x+b.w-60,b.y+b.h-20,60,20),_t('ok'),1)||ev.exit)modal_exit(0)
		draw_text(b,`Decker v${VERSION}`,FONT_MENU,1),b.y+=15
		draw_text(b,_t('about_author'),FONT_BODY,1),b.y+=12
		draw_text(b,_t('about_website'),FONT_BODY,1),b.y+=13
		draw_text(b,_t('translation_credit'),FONT_MENU,1),b.y+=15
		draw_text(b,'雪风与梦',FONT_MENU,1),b.y+=15
		draw_text(b,'角赤pulupo',FONT_MENU,1),b.y+=15
	}
		...
		draw_textc(rect(b.x,b.y-5,b.w,20),_t('cards'),FONT_MENU,1),draw_box(gsize,0,1)
		...
		if(ui_button(rect(b.x+b.w-60,b.y+b.h-20,60,20),_t('ok'),1)||ev.exit||ev.action)modal_exit(0)
		const c=rect(b.x,b.y+b.h-20)
		if(ui_button(rect(c.x,c.y,60,20),_t('new'),1)){
		...
		else if(ev.dir=='right'||ev.dir=='down'){m=1,n_go([_tlms('next_card')],deck)}
		if(m){
		...
		draw_textc(rect(b.x,b.y-5,b.w,20),_t('widget_order'),FONT_MENU,1)
		...
		if(ui_button(rect(b.x+b.w-60,b.y+b.h-20,60,20),_t('ok'),1)||ev.exit)modal_exit(0)
		const c=rect(b.x,b.y+b.h-20)
		if(ui_button(rect(c.x,c.y,80,20),_t('properties'),ob.sel.length==1)||props)if(ob.sel.length)object_properties(ob.sel[0])
		if(ev.mu){
			if(ms.grid.row!=-1&&gutter!=-1){
		...
		const vol_h=20, vol_gap=5
		const b=draw_modalbox(rect(250,frame.size.y-16-30)), gsize=rect(b.x,b.y+15,b.w,b.h-16-(2*25)-vol_h-vol_gap)
		draw_textc(rect(b.x,b.y-5,b.w,20),_t('sounds'),FONT_MENU,1)
		...
		{
			const slider={
				size:rint(rect(b.x,gsize.y+gsize.h+vol_gap,b.w,vol_h)),
				font:FONT_BODY,
				show:'solid',
				locked:0,
				value:sfx_volume_percent,
				min:SFX_VOLUME_MIN,
				max:SFX_VOLUME_MAX,
				step:1,
				style:'bar',
				format:`${_t('volume')}: %0.0f%%`,
			}
			widget_slider(null,slider)
			const next=0|slider.value
			if(next!=sfx_volume_percent){
				sfx_volume_percent=next
				try{localStorage.setItem('wpaint_sfx_volume_percent',''+next)}catch(e){}
			}
		}
		if(ui_button(rect(b.x+b.w-60,b.y+b.h-20,60,20),_t('ok'),1)||ev.exit){
			if(ms.from_action){
		...
		const c=rect(b.x,b.y+b.h-20)
		if(ui_button(rect(c.x,c.y,60,20),_t('edit')+'...',s!=null))au.target=s,modal_enter('recording')
		if(ui_button(rect(c.x,c.y-25,60,20),_t('new')+'...',1))au.target=deck_add(deck,lms('sound')),mark_dirty(),modal_enter('recording');c.x+=65
		if(ui_button(rect(c.x,c.y,60,20),_t('delete'),s!=null))deck_remove(deck,s),mark_dirty(),ms.grid=gridtab(sounds_enumerate())
	}
	else if(ms.type=='recording'){
		const b=draw_modalbox(rect(frame.size.x-50,130)), samples=max(1,ln(ifield(au.target,'size')))
		draw_textc(rect(b.x,b.y-5,b.w,20),_t('audio_editor'),FONT_MENU,1)
		...
		if(ui_button(rect(b.x+b.w-60,b.y+b.h-20,60,20),_t('ok'),1)||ev.exit)modal_exit(0)
		const c=rect(b.x,b.y+b.h-20)
		if(ui_toggle(rect(c.x,c.y,60,20),_t('play'),au.mode=='playing',1)){
		...
		if(ui_toggle(rect(c.x,c.y,60,20),_t('record'),au.mode=='recording',!au.norecord)){
		...
		draw_text(rect(b.x+(b.w/2),gsize.y+gsize.h+9,37,20),_t('name'),FONT_MENU,1)
		...
		draw_textc(rect(b.x,b.y-5,b.w,20),_t('fonts'),FONT_MENU,1)
		...
		if(ui_button(rect(c.x,c.y,60,20),_t('ok'),ms.grid.row>=0)||choose){
		...
		if(ui_button(rect(c.x,c.y,60,20),_t('cancel'),1)||ev.exit)modal_pop(0)
	}
	else if(ms.type=='contraptions'||ms.type=='pick_contraption'){
		const b=draw_modalbox(rect(250,230))
		draw_textc(rect(b.x,b.y-5,b.w,20),ms.type=='contraptions'?_t('contraption_prototype'):_t('new_contraption'),FONT_MENU,1)
		...
		if(ms.type=='contraptions'){
			if(ui_button(rect(b.x+b.w-60,b.y+b.h-20,60,20),_t('ok'),1)||ev.exit){modal_exit(0)}
			if(ui_button(rect(b.x,b.y+b.h-45,60,20),_t('new')+'...',1)){modal_exit(1),con_set(deck_add(deck,lms('contraption'))),mark_dirty()}
			if(ui_button(rect(b.x,b.y+b.h-20,60,20),_t('edit')+'...',ms.grid.row>=0)||choose){modal_exit(2),con_set(deck.contraptions.v[ms.grid.row])}
			if(ui_button(rect(b.x+65,b.y+b.h-45,60,20),_t('clone'),ms.grid.row>=0)){
			...
			if(ui_button(rect(b.x+65,b.y+b.h-20,60,20),_t('delete'),ms.grid.row>=0)){
				deck_remove(deck,ifield(deck,'contraptions').v[ms.grid.row])
				ms.grid=gridtab(contraptions_enumerate())
			}
		}
		if(ms.type=='pick_contraption'){
			if(ui_button(rect(c.x,c.y,60,20),_t('create'),ms.grid.row>=0)||choose){
				ob_create([lmd(['type','def'].map(lms),[lms('contraption'),tab_cell(ms.grid.table,'name',ms.grid.row)])]),modal_exit(1)
			};c.x-=65
			if(ui_button(rect(c.x,c.y,60,20),_t('cancel'),1)||ev.exit)modal_exit(0)
		}
	}
	else if(ms.type=='contraption_props'){
		const b=draw_modalbox(rect(240,240)),contraption=ob.sel[0]
		draw_textc(rect(b.x,b.y-5,b.w,20),_t('contraption_properties',{name:title_caps(contraption.def.name)}),FONT_MENU,1)
		draw_text(rect(b.x,b.y+22,47,20),_t('name'),FONT_MENU,1)
		...
		const c=rect(b.x,b.y+b.h-20)
		if(ui_button(rect(c.x,c.y,60,20),_t('script'),1))modal_exit(0),setscript(contraption);c.x+=65
		if(ui_button(rect(c.x,c.y,80,20),_t('prototype'),1))modal_exit(1),con_set(contraption.def)
		if(ui_button(rect(b.x+b.w-60,c.y,60,20),_t('ok'),1)||ev.exit)modal_exit(1)
	}
	else if(ms.type=='prototype_props'){
		const b=draw_modalbox(rect(220,230)),def=con(), lw=67
		draw_textc(rect(b.x,b.y-5,b.w,20),_t('prototype_properties'),FONT_MENU,1)
		draw_text(rect(b.x,b.y+ 22,lw,20),_t('name'),FONT_MENU,1)
		draw_text(rect(b.x,b.y+ 42,lw,20),_t('version'),FONT_MENU,1)
		draw_text(rect(b.x,b.y+ 62,lw,20),_t('description'),FONT_MENU,1)
		draw_text(rect(b.x,b.y+122,lw,20),_t('template_script'),FONT_MENU,1)
		...
		const c=rect(b.x,b.y+b.h-20)
		if(ui_button(rect(c.x,c.y,60,20),_t('script'),1))modal_exit(1),setscript(def);c.x+=65
		if(ui_button(rect(b.x+b.w-60,c.y,60,20),_t('ok'),1)||ev.exit)modal_exit(0)
	}
	else if(ms.type=='prototype_attrs'){
		const b=draw_modalbox(rect(220,200)),def=con(), lw=42
		draw_textc(rect(b.x,b.y-5,b.w,20),`${title_caps(def.name)} 特性`,FONT_MENU,1)
		...
		draw_text(rect(gsize.x+gsize.w+10,b.y+20+2,lw,20),_t('name') ,FONT_MENU,1)
		draw_text(rect(gsize.x+gsize.w+10,b.y+40+2,lw,20),_t('label'),FONT_MENU,1)
		draw_text(rect(gsize.x+gsize.w+10,b.y+60+2,lw,20),_t('type') ,FONT_MENU,1)
		...
		const attr_labels=[_t('attr_boolean'),_t('attr_number'),_t('attr_string'),_t('attr_code'),_t('attr_richtext')]
		...
		if(ui_button(rect(c.x,c.y,60,20),_t('add'),1)){
		...
		if(ui_button(rect(c.x,c.y,60,20),_t('remove'),sel)){
			ms.grid.table=dyad.drop(lml([lmn(ms.grid.row)]),ms.grid.table)
			ms.grid.row=-1,ms.name.table=null,ms.text.table=null
		}
		if(ui_button(rect(b.x+b.w-60,c.y,60,20),_t('ok'),1)||ev.exit){iwrite(def,lms('attributes'),ms.grid.table),mark_dirty(),modal_exit(1)}
	}
	else if(ms.type=='resources'){
		const b=draw_modalbox(rect(380,190))
		draw_textc(rect(b.x,b.y-5,b.w,20),_t('font_deck_mover'),FONT_MENU,1)
		const lgrid=rect(b.x            ,b.y+15 ,120    ,b.h-(15+15+5+20))
		const rgrid=rect(b.x+b.w-lgrid.w,lgrid.y,lgrid.w,lgrid.h         )
		if(ui_button(rect(rgrid.x+(rgrid.w-80)/2,b.y+b.h-20,80,20),_t('ok'),1)||ev.exit)modal_exit(0)
		...
		draw_textc(rect(lgrid.x,lgrid.y+lgrid.h+3,lgrid.w,15),ms.message?ls(ifield(ms.message,'name')):'(选择一个卡组)',FONT_BODY,1)
		...
		let copy_message='>> 复制 >>',can_copy=1
		if(ms.grid.row>-1&&sel&&(module_is(sel)||prototype_is(sel))){
			...
			if(sver>dver)can_copy=1,copy_message='>> 升级 >>'
			if(sver<dver)can_copy=1,copy_message='>> 降级 >>'
		}
		...
		if(ui_button(cb,_t('remove'),ms.grid2.row>-1)){
		...
		if(sel&&font_is(sel)){
			draw_textc(rect(pre.x,pre.y+pre.h-18,pre.w,18),count(ifield(sel,'glyphs'))+' 字形',FONT_BODY,1);pre.h-=20
		...
		if(sel&&sound_is(sel)){if(ui_button(cb,_t('play'),1))n_play([sel])}
		...
		ui_button(rint(rect(lgrid.x+(lgrid.w-80)/2,b.y+b.h-20,80,20)),_t('choose'),1,_=>open_text('.html,.deck',text=>{
		...
		const desc=t?_t('query_result',{cols:tab_cols(t).length,cols_plural:tab_cols(t).length==1?'':'s',rows:count(t),rows_plural:count(t)==1?'':'s'}):_t('executing_query')
		...
		const c=rect(b.x+b.w-60,b.y+b.h-20)
		if(ui_button(rect(c.x,c.y,60,20),_t('run'),compiles)||ev.eval){
			try{
		...
		if(ui_button(rect(c.x,c.y,60,20),_t('apply'),ms.grid.table!=null&&ms.grid.table!=ms.old_wid.gv.table&&!ms.old_wid.g.locked)){
		...
		if(ui_button(rect(c.x,c.y,60,20),_t('close'),1)||ev.exit)modal_exit(0)
		...
		else{draw_box(gsize,0,1),draw_textc(gsize,_t('working'),FONT_BODY,1)}
	}
	else if(ms.type=='url'){
		const b=draw_modalbox(rect(200,90))
		draw_textc(rect(b.x,b.y,b.w,20),_t('open_url_prompt'),FONT_BODY,1)
		ui_textedit(rect(b.x,b.y+20+5,b.w,40),1,ms.text)
		const c=rect(b.x+b.w-(b.w-(2*60+5))/2-60,b.y+b.h-20)
		ui_button(rect(c.x,c.y,60,20),_t('open'),1,_=>{open_url(ls(rtext_string(ms.text.table)));modal_exit(0)});c.x-=65
		if(ui_button(rect(c.x,c.y,60,20),_t('cancel'),1)||ev.exit)modal_exit(0)
	}
	else if(ms.type=='link'){
		const b=draw_modalbox(rect(230,70))
		draw_textc(rect(b.x,b.y,b.w,20),kc.heading=_t('enter_link'),FONT_BODY,1)
		ui_field(rect(b.x,b.y+20+5,b.w,20),ms.text)
		const c=rect(b.x+b.w-60,b.y+b.h-20)
		if(ui_button(rect(c.x,c.y,60,20),_t('ok'),1))modal_pop(1);c.x-=65
		if(ui_button(rect(c.x,c.y,60,20),_t('cancel'),1)||ev.exit)modal_pop(0)
		if(ui_button(rect(b.x,c.y,60,20),_t('cards_menu'),1))modal_push('pick_card')
	}
		...
		draw_textc(rect(b.x,b.y-5,b.w,20),_t('save_file'),FONT_MENU,1),draw_text_wrap(tbox,l,1)
		draw_text(rect(b.x   ,tbox.y+tbox.h+7,    56,20),_t('filename'),FONT_MENU,1)
		ui_field (rect(b.x+56,tbox.y+tbox.h+5,b.w-56,18),ms.text)
		const c=rect(b.x+b.w-60,b.y+b.h-20),subtype=ms.subtype
		if(ui_button(rect(c.x,c.y,60,20),_t('save'),1,_=>{
		...
		const b=draw_modal_rtext(rect(0,5+20)),v=ms.verb?ls(ms.verb):_t('ok')
		const vs=font_textsize(FONT_MENU,v);vs.x=min(max(60,vs.x+10),200-65);const c=rect(b.x+b.w-vs.x,b.y+b.h-20)
		if(ui_button(rect(c.x,c.y,vs.x,20),v,1))modal_exit(1)
		const cancel_text=ms.cancel_text?ls(ms.cancel_text):_t('cancel')
		const cancel_vs=font_textsize(FONT_MENU,cancel_text);const cancel_w=min(max(60,cancel_vs.x+10),c.x-5)
		c.x-=cancel_w+5
		if(ui_button(rect(c.x,c.y,cancel_w,20),cancel_text,1)||ev.exit)modal_exit(0)
	}
		...
		if(ui_button(rect(b.x+b.w-60,b.y+b.h-20,60,20),_t('ok'),ms.grid.row>=0)||choose)modal_exit(1)
	}
	else if(ms.type=='open_lil'){
		const b=draw_modalbox(rect(125,45))
		draw_textc(rect(b.x,b.y,b.w,16),ms.prompt||_t('click_to_open'),FONT_BODY,1)
		const c=rect(b.x+b.w-60,b.y+b.h-20)
		ui_button(rect(c.x,c.y,60,20),_t('open'),1,_=>{
			...
			else if(ms.filter=='.gif'       )open_file(ms.filter,file=>{load_image(file,ms.verb,image=>{arg(),ret(image)          ,modal_exit(1)})})
		...
		draw_textc(rect(b.x,b.y,b.w,16),_t('click_fullscreen'),FONT_BODY,1)
		const c=rect(b.x+b.w-80,b.y+b.h-20)
		ui_button(rect(c.x,c.y,80,20),_t('fullscreen'),1,_=>{toggle_fullscreen(),modal_exit(1)}),c.x-=65
		if(ui_button(rect(c.x,c.y,60,20),_t('cancel'),1)||ev.exit)modal_exit(0)
	}
		...
		const lab=dr.brush>=(6*4)?ls(br.k[dr.brush-(6*4)]):_t('select_brush_shape')
		...
		draw_textc(rect(b.x,b.y+b.h-lh,b.w,lh),_t('select_fill_stroke_color',{type:ms.type=='fill'?_t('fill'):_t('stroke'),color_or_pattern:dr.color?_t('color'):_t('pattern')}),FONT_BODY,1)
		for(let z=0;z<grid.x*grid.y;z++){
		...
		draw_textc(rect(b.x,b.y-5,b.w,20),_t('grid_size'),FONT_MENU,1)
		draw_text(rect(b.x,b.y+22,42,20),_t('width') ,FONT_MENU,1)
		draw_text(rect(b.x,b.y+42,42,20),_t('height'),FONT_MENU,1)
		...
		draw_textc(rect(zc.x,zc.y,b.w,20),_t('pixel_scale'),FONT_MENU,1),zc.y+=20
		...
		draw_textc(rect(b.x,b.y-5,b.w,20),_t('deck_properties'),FONT_MENU,1)
		draw_text(rect(b.x,b.y+22,42,20),_t('name'),FONT_MENU,1)
		draw_text(rect(b.x,b.y+42,42,20),_t('author'),FONT_MENU,1)
		// draw_text(rect(b.x,b.y+62,42,20),'宽度',FONT_MENU,1)
		// draw_text(rect(b.x,b.y+82,42,20),'高度',FONT_MENU,1)
		ui_field(rect(b.x+42,b.y+20,b.w-42,18),ms.name)
		ui_field(rect(b.x+42,b.y+40,b.w-42,30),ms.text)
		// ui_field(rect(b.x+42,b.y+60,b.w-42,18),ms.form0)
		// ui_field(rect(b.x+42,b.y+80,b.w-42,18),ms.form1)
		iwrite(deck,lms('name'  ),rtext_string(ms.name.table))
		iwrite(deck,lms('author'),rtext_string(ms.text.table))
		mark_dirty()
		const c=rect(b.x,b.y+b.h-20)
		if(ui_button(rect(c.x,c.y,60,20),_t('script'),1))setscript(deck),modal_exit(0);c.x+=65
		if(ui_button(rect(c.x,c.y,60,20),_t('protect')+'...',1))modal_enter('save_locked')
		if(ui_button(rect(b.x+b.w-60,c.y,60,20),_t('ok'),1)||ev.exit){
			// // 应用新的deck大小
			// const width = parseInt(rtext_string(ms.form0.table)) || 512
			// const height = parseInt(rtext_string(ms.form1.table)) || 342
			// deck.size = [width, height]
			// // 重新创建framebuffer和更新显示
	...
	}	else if(ms.type=='resize_deck'){
		const b=draw_modalbox(rect(280,140))
		draw_textc(rect(b.x,b.y-5,b.w,20),_t('resize_deck_title'),FONT_MENU,1)
		draw_text(rect(b.x,b.y+22,42,20),_t('width'),FONT_MENU,1)
		draw_text(rect(b.x,b.y+42,42,20),_t('height'),FONT_MENU,1)
		// draw_text(rect(b.x,b.y+62,62,20),'调整完，需要手动到组件调整画布大小，才能绘画',FONT_MENU,1)
		ui_field(rect(b.x+42,b.y+20,b.w-42,18),ms.form0)
		ui_field(rect(b.x+42,b.y+40,b.w-42,18),ms.form1)
		// 靠边复选框
		let checkVal = parseInt(rtext_string(ms.form2.table).v) || 0
		if(ui_checkbox(rect(b.x,b.y+82,b.w,16),_t('adaptive'),1,checkVal)){
			checkVal^=1
			ms.form2.table = rtext_cast(lms(checkVal+''))
		}
		const c=rect(b.x,b.y+b.h-20)
		if(ui_button(rect(c.x,c.y,60,20),_t('cancel'),1))modal_exit(0)
		if(ui_button(rect(b.x+b.w-60,c.y,60,20),_t('ok'),1)||ev.exit){
// 获取新的尺寸和靠边选项
			const newWidth = parseInt(rtext_string(ms.form0.table).v) || 512
			const newHeight = parseInt(rtext_string(ms.form1.table).v) || 342
			const keepEdge = parseInt(rtext_string(ms.form2.table).v) || 0
			
			// 使用保存的deck数据重新创建deck，但使用新的尺寸
			const tempDeck = deck_read(ms.temp_deck_data)
			const oldSize = tempDeck.size || rect(512,342)
			tempDeck.size = rect(newWidth, newHeight)
			
			// 如果勾选了靠边选项，调整元素位置和背景图像
			if(keepEdge){
				const adjustElements = (widgets) => {
					widgets.forEach(widget => {
						if(widget.pos && widget.size){
							const rightDist = oldSize.x - (widget.pos.x + widget.size.x)
							const bottomDist = oldSize.y - (widget.pos.y + widget.size.y)
							// 如果距离右边缘小于146像素
							if(rightDist < 146 || bottomDist < 54){
								widget.pos.x = newWidth - widget.size.x - rightDist
								widget.pos.y = newHeight - widget.size.y - bottomDist
							}
						}
					})
				}
				...
				// 调整背景图像
				const adjustBackgroundImage = (card) => {
					if(card.image && card.image.pix && card.image.size){
						const oldW = card.image.size.x
						const oldH = card.image.size.y
						const newW = newWidth
						const newH = newHeight
						
						if(newW != oldW || newH != oldH){
						const oldPix = card.image.pix
						const newPix = new Uint8Array(newW * newH)
						
						// 第一步：逐行转移旧数据到新数据
						for(let y = 0; y < Math.min(oldH, newH); y++){
							// 复制旧数据的这一行
							for(let x = 0; x < Math.min(oldW, newW); x++){
								newPix[y * newW + x] = oldPix[y * oldW + x]
							}
							// 行末新增部分填0（仅在增大时）
						...
						for(let y = oldH; y < newH; y++){
							...
							// 第二步：移动边缘像素（L形区域整体移动）
							const rightEdgeRange = 146
							const bottomEdgeRange = 54
							
							// 计算L形区域的范围
							const sourceStartX = Math.max(0, oldW - rightEdgeRange)
							const sourceStartY = Math.max(0, oldH - bottomEdgeRange)
							const sourceWidth = oldW - sourceStartX
							const sourceHeight = oldH - sourceStartY
							
							// 目标位置（右下角）
							const targetStartX = newW - sourceWidth
							const targetStartY = newH - sourceHeight
							
							// L形区域始终移动到右下角，保持完整性
					if(sourceWidth > 0 && sourceHeight > 0){
								// 移动L形区域到右下角作为整体，无论增大还是缩小
								
								// 1. 移动右边缘部分（完整的右边缘条带）
							for(let y = 0; y < oldH; y++){
								for(let x = 0; x < sourceWidth; x++){
									const sourceIdx = y * oldW + (sourceStartX + x)
									// 右边缘移动到新画布右下角对齐
									const targetY = Math.max(0, newH - oldH + y)
									const targetX = newW - sourceWidth + x
									const targetIdx = targetY * newW + targetX
									if(sourceIdx < oldPix.length && targetY < newH && targetX >= 0 && targetX < newW){
										newPix[targetIdx] = oldPix[sourceIdx]
									}
								}
							}
								
								// 2. 移动下边缘部分（完整的下边缘条带，但排除已移动的右边缘重叠部分）
							for(let y = 0; y < sourceHeight; y++){
								for(let x = 0; x < sourceStartX; x++){
									const sourceIdx = (sourceStartY + y) * oldW + x
									// 下边缘移动到新画布右下角对齐，在宽度减小时左移
									const targetY = newH - sourceHeight + y
									const targetX = newW - oldW + x
									const targetIdx = targetY * newW + targetX
									if(sourceIdx < oldPix.length && targetY >= 0 && targetY < newH && targetX >= 0 && targetX < newW){
										newPix[targetIdx] = oldPix[sourceIdx]
									}
								}
							}
							...
							// 更新图像数据
							card.image.pix = newPix
							card.image.size.x = newW
							card.image.size.y = newH
							
							// 计算尺寸差值
							const deltaW = newW - oldW
							const deltaH = newH - oldH
							
							// 同时调整target和canvas widget的相关属性
							if(card.widgets && card.widgets.v){
								card.widgets.v.forEach(widget => {
									if(widget.name === 'target'){
										if(widget.size){
											widget.size.x += deltaW
											widget.size.y += deltaH
										}
										if(widget.clip){
											widget.clip.w += deltaW
											widget.clip.h += deltaH
										}
										// if(widget.image) {
										// 	const newSize = rect(widget.image.size.x + deltaW, widget.image.size.y + deltaH);
										// 	image_resize(widget.image, newSize); // 显式调用 resize
										// 	canvas_clip(widget); // 调整完大小后，重置裁剪区域
										// }
										  	const innerCanvas = dget(widget.widgets, lms('c'));

											if (innerCanvas && canvas_is(innerCanvas)) {
												
												// 3. 调整内部画布 'c' 的尺寸
												if (innerCanvas.size) {
													innerCanvas.size.x += deltaW;
													innerCanvas.size.y += deltaH;
												}
												
												// 4. 【核心修复】获取并调整 'c' 画布的图像数据
												let canvasImage = container_image(innerCanvas, true); // 获取或创建 image 对象
												if (canvasImage) {
													image_resize(canvasImage, rect(innerCanvas.size.x, innerCanvas.size.y));
												}
												
												// 5. 更新内部画布 'c' 的裁剪区域
												canvas_clip(innerCanvas);
											}
									}
  									else if(widget.name === 'canvas'){
										if(widget.size){
											widget.size.x += deltaW
											widget.size.y += deltaH
										}
										...
										if(widget.image && widget.image.size){
											widget.image.size.x += deltaW
											widget.image.size.y += deltaH
										}
										// 重新计算canvas的裁剪区域，确保新增区域可以绘制
										canvas_clip(widget)
									}
								})
							}
				...
				// 调整所有卡片中的元素
				if(tempDeck.cards && tempDeck.cards.v && Array.isArray(tempDeck.cards.v)){
					tempDeck.cards.v.forEach(card => {
						if(card.widgets && card.widgets.v) adjustElements(card.widgets.v)
					})
				}
				
				// 调整当前激活卡片的背景图像
				if(tempDeck.cards && tempDeck.cards.v && tempDeck.card !== undefined){
					const currentCard = tempDeck.cards.v[tempDeck.card]
					if(currentCard) adjustBackgroundImage(currentCard)
				}
			}
		...
		draw_textc(rect(b.x,b.y-5,b.w,20),_t('card_properties'),FONT_MENU,1)
		draw_text(rect(b.x,b.y+22,42,20),_t('name'),FONT_MENU,1),ui_field(rect(b.x+42,b.y+20,b.w-42,18),ms.name)
		const c=rect(b.x,b.y+b.h-20)
		if(ui_button(rect(b.x+b.w-60,c.y,60,20),_t('ok'),1)||ev.exit)modal_exit(1)
		if(ui_button(rect(c.x,c.y,60,20),_t('script'),1))setscript(card),modal_exit(0)
	}
	else if(ms.type=='button_props'){
		const b=draw_modalbox(rect(220,170)),button=ob.sel[0]
		draw_textc(rect(b.x,b.y-5,b.w,20),_t('button_attributes'),FONT_MENU,1)
		draw_text(rect(b.x,b.y+22,42,20),_t('name'),FONT_MENU,1)
		draw_text(rect(b.x,b.y+42,42,20),_t('text'),FONT_MENU,1)
		ui_field(rect(b.x+42,b.y+20,b.w-42,18),ms.name)
		ui_field(rect(b.x+42,b.y+40,b.w-42,18),ms.text)
		iwrite(button,lms('name'),rtext_string(ms.name.table))
		iwrite(button,lms('text'),rtext_string(ms.text.table))
		draw_text(rect(b.x+(0|(b.w/2)),b.y+72,54,20),_t('shortcut'),FONT_MENU,1)
		...
		if(ui_radio(rint(rect(sb.x,sb.y,b.w/2,16)),_t('button_style_round')    ,1,style=='round'    )){iwrite(button,lms('style'),lms('round'    )),mark_dirty()}sb.y+=16
		if(ui_radio(rint(rect(sb.x,sb.y,b.w/2,16)),_t('button_style_rect'),1,style=='rect'     )){iwrite(button,lms('style'),lms('rect'     )),mark_dirty()}sb.y+=16
		if(ui_radio(rint(rect(sb.x,sb.y,b.w/2,16)),_t('button_style_check') ,1,style=='check'    )){iwrite(button,lms('style'),lms('check'    )),mark_dirty()}sb.y+=16
		if(ui_radio(rint(rect(sb.x,sb.y,b.w/2,16)),_t('button_style_invisible'),1,style=='invisible')){iwrite(button,lms('style'),lms('invisible')),mark_dirty()}sb.y+=16
		const c=rect(b.x,b.y+b.h-20)
		if(ui_button(rect(c.x,c.y,60,20),_t('script'),1))setscript(button),modal_exit(0); c.x+=65
		if(ui_button(rect(c.x,c.y,60,20),_t('action')+'...',card_is(con())))modal_enter('action')
		if(ui_button(rect(b.x+b.w-60,c.y,60,20),_t('ok'),1)||ev.exit)modal_exit(1)
	}
	else if(ms.type=='field_props'){
		const b=draw_modalbox(rect(260,200+60)),f=ob.sel[0], p=unpack_field(f)
		draw_textc(rect(b.x,b.y-5,b.w,20),_t('field_attributes'),FONT_MENU,1)
		draw_text(rect(b.x,b.y+22,42,20),_t('name'),FONT_MENU,1)
		draw_text(rect(b.x,b.y+42,42,60),_t('text'),FONT_MENU,1)
		ui_field(rect(b.x+42,b.y+20,b.w-42,18),ms.name)
		...
		if(ui_checkbox(rect(cb.x,cb.y,b.w,16),_t('border')   ,1,border   )){border   ^=1,iwrite(f,lms('border'   ),lmn(border   )),mark_dirty()}cb.y+=16
		if(ui_checkbox(rect(cb.x,cb.y,b.w,16),_t('scrollbar'),1,scrollbar)){scrollbar^=1,iwrite(f,lms('scrollbar'),lmn(scrollbar)),mark_dirty()}cb.y+=16
		const sb=rect(b.x,cb.y+10);let cp=0
		if(ui_radio(rint(rect(sb.x,sb.y,b.w/2,16)),_t('attr_richtext') ,1,style=='rich' )){iwrite(f,lms('style'),lms('rich' )),mark_dirty()     }sb.y+=16
		if(ui_radio(rint(rect(sb.x,sb.y,b.w/2,16)),'Plain Text',1,style=='plain')){iwrite(f,lms('style'),lms('plain')),mark_dirty(),cp=1}sb.y+=16
		if(ui_radio(rint(rect(sb.x,sb.y,b.w/2,16)),_t('attr_code')      ,1,style=='code' )){iwrite(f,lms('style'),lms('code' )),mark_dirty(),cp=1}sb.y+=16
		...
		if(ui_radio(rint(rect(ab.x,ab.y,b.w/2,16)),'Left' ,1,align=='left'  )){iwrite(f,lms('align'),lms('left'  )),mark_dirty()}ab.y+=16
		if(ui_radio(rint(rect(ab.x,ab.y,b.w/2,16)),'Center'     ,1,align=='center')){iwrite(f,lms('align'),lms('center')),mark_dirty()}ab.y+=16
		if(ui_radio(rint(rect(ab.x,ab.y,b.w/2,16)),'Right',1,align=='right' )){iwrite(f,lms('align'),lms('right' )),mark_dirty()}ab.y+=16
		const c=rect(b.x,b.y+b.h-20)
		if(ui_button(rect(c.x,c.y,60,20),_t('script')+'...',1))setscript(f),modal_exit(0)
		if(ui_button(rect(b.x+b.w-60,c.y,60,20),_t('ok'),1)||ev.exit)modal_exit(1)
	}
	else if(ms.type=='slider_props'){
		const b=draw_modalbox(rect(220,170)),f=ob.sel[0]
		draw_textc(rect(b.x,b.y-5,b.w,20),_t('slider_attributes'),FONT_MENU,1)
		draw_text(rect(b.x,b.y+22,42,20),_t('name')  ,FONT_MENU,1)
		draw_text(rect(b.x,b.y+42,42,20),_t('format'),FONT_MENU,1)
		...
		if(ui_radio(rint(rect(sb.x,sb.y,b.w/2,16)),_t('slider_style_horiz'),1,style=='horiz'  )){iwrite(f,lms('style'),lms('horiz'  )),mark_dirty()}sb.y+=16
		if(ui_radio(rint(rect(sb.x,sb.y,b.w/2,16)),_t('slider_style_vert')  ,1,style=='vert'   )){iwrite(f,lms('style'),lms('vert'   )),mark_dirty()}sb.y+=16
		if(ui_radio(rint(rect(sb.x,sb.y,b.w/2,16)),_t('slider_style_bar')       ,1,style=='bar'    )){iwrite(f,lms('style'),lms('bar'    )),mark_dirty()}sb.y+=16
		if(ui_radio(rint(rect(sb.x,sb.y,b.w/2,16)),_t('slider_style_compact')   ,1,style=='compact')){iwrite(f,lms('style'),lms('compact')),mark_dirty()}sb.y+=16
		const ib=rint(rect(b.x+b.w/2,b.y+70))
		draw_text(rect(ib.x+5,ib.y+2,40,20),_t('min_value') ,FONT_MENU,1),ui_field(rint(rect(ib.x+40,ib.y,b.w/2-40,18)),ms.form0),ib.y+=20
		draw_text(rect(ib.x+5,ib.y+2,40,20),_t('max_value') ,FONT_MENU,1),ui_field(rint(rect(ib.x+40,ib.y,b.w/2-40,18)),ms.form1),ib.y+=20
		draw_text(rect(ib.x+5,ib.y+2,40,20),_t('step'),FONT_MENU,1),ui_field(rint(rect(ib.x+40,ib.y,b.w/2-40,18)),ms.form2),ib.y+=20
		const c=rect(b.x,b.y+b.h-20)
		...
		const b=draw_modalbox(rect(220,181)),canvas=ob.sel[0]
		draw_textc(rect(b.x,b.y-5,b.w,20),_t('canvas_attributes'),FONT_MENU,1)
		draw_text(rect(b.x,b.y+22,42,20),_t('name') ,FONT_MENU,1)
		draw_text(rect(b.x,b.y+42,42,20),_t('scale'),FONT_MENU,1)
		draw_text(rect(b.x,b.y+62,42,20),_t('width'),FONT_MENU,1)
		draw_text(rect(b.x,b.y+82,42,20),_t('height'),FONT_MENU,1)
		ui_field(rect(b.x+42,b.y+20,b.w-42,18),ms.name)
		ui_field(rect(b.x+42,b.y+40,b.w-42,18),ms.text)
		ui_field(rect(b.x+42,b.y+60,b.w-42,18),ms.form0)
		ui_field(rect(b.x+42,b.y+80,b.w-42,18),ms.form1)
		...
		// 处理画布大小设置
		const newWidth = parseInt(rtext_string(ms.form0.table))
		const newHeight = parseInt(rtext_string(ms.form1.table))
		if(!isNaN(newWidth) && !isNaN(newHeight) && newWidth > 0 && newHeight > 0){
			const newSize = rect(newWidth, newHeight)
			iwrite(canvas,lms('size'),lmpair(newSize))
			mark_dirty()
		}
		let border=lb(ifield(canvas,'border')),draggable=lb(ifield(canvas,'draggable')),cb=rect(b.x,b.y+90+20)
		if(ui_checkbox(rint(rect(cb.x,cb.y,b.w/2,16)),_t('border')   ,1,border   ))border   ^=1,iwrite(canvas,lms('border'   ),lmn(border   )),mark_dirty();cb.y+=16
		if(ui_checkbox(rect(cb.x,cb.y,b.w,16),_t('draggable'),1,draggable))draggable^=1,iwrite(canvas,lms('draggable'),lmn(draggable)),mark_dirty()
		const c=rect(b.x,b.y+b.h-20)
		if(ui_button(rect(c.x,c.y,60,20),_t('script'),1))setscript(canvas),modal_exit(0)
		if(ui_button(rect(b.x+b.w-60,c.y,60,20),_t('ok'),1)||ev.exit)modal_exit(1)
	}
	else if(ms.type=='grid_props'){
		const b=draw_modalbox(rect(280,216+70)),grid=ob.sel[0]
		draw_textc(rect(b.x,b.y-5,b.w,20),_t('grid_properties'),FONT_MENU,1)
		draw_text(rect(b.x,b.y+22,47,20),_t('name')  ,FONT_MENU,1)
		draw_text(rect(b.x,b.y+42,47,20),_t('format'),FONT_MENU,1)
		draw_text(rect(b.x,b.y+62,47,20),_t('value') ,FONT_MENU,1)
		ui_field   (rect(b.x+47,b.y+20,b.w-47,18),ms.name)
		...
		draw_text(rect(b.x+47,b.y+120+60,b.w-47,18),`${cn} 列${cn==1?'':''}，${rn} 行${rn==1?'':''}。`,FONT_BODY,1)
		...
		if(ui_checkbox(rint(rect(cb.x,cb.y,b.w/2,16)),_t('headers'),1,headers  )){headers  ^=1,iwrite(grid,lms('headers'  ),lmn(headers  )),mark_dirty()}cb.y+=16
		if(ui_checkbox(rint(rect(cb.x,cb.y,b.w/2,16)),_t('scrollbar')     ,1,scrollbar)){scrollbar^=1,iwrite(grid,lms('scrollbar'),lmn(scrollbar)),mark_dirty()}cb.y+=16
		if(ui_checkbox(rint(rect(cb.x,cb.y,b.w/2,16)),_t('grid_lines')    ,1,lines    )){lines    ^=1,iwrite(grid,lms('lines'    ),lmn(lines    )),mark_dirty()}cb.y+=16
		if(ui_checkbox(rint(rect(cb.x,cb.y,b.w/2,16)),_t('select_by_cell'),1,bycell   )){bycell   ^=1,iwrite(grid,lms('bycell'   ),lmn(bycell   )),mark_dirty()}cb.y+=16
		const eb=rint(rect(b.x+(b.w/2),b.y+130+70))
		if(ui_radio(rint(rect(eb.x,eb.y,b.w/2,16)),_t('edit_as_json'),1,ms.edit_json==1)){ms.form0=fieldstr(lms(fjson(monad.cols(val)))),ms.edit_json=1}eb.y+=16
		if(ui_radio(rint(rect(eb.x,eb.y,b.w/2,16)),_t('edit_as_csv') ,1,ms.edit_json==0))ms.form0=fieldstr(n_writecsv(count(format)?[val,format]:[val])),ms.edit_json=0
		const c=rect(b.x,b.y+b.h-20), w=ll(ifield(grid,'widths'))
		if(ui_button(rect(c.x,c.y,60,20),_t('script'),1))modal_exit(0),setscript(grid);c.x+=65
		if(ui_button(rect(c.x,c.y,90,20),_t('reset_widths'),w.length))iwrite(grid,lms('widths'),lml([])),mark_dirty()
		if(ui_button(rect(b.x+b.w-60,c.y,60,20),_t('ok'),1)||ev.exit)modal_exit(1)
	}
	else if(ms.type=='action'){
		const b=draw_modalbox(rect(220,190))
		draw_textc(rect(b.x,b.y-5,b.w,20),_t('button_action'),FONT_MENU,1)
		...
		if(ui_button(rect(c.x,c.y,60,20),_t('ok'),ready))modal_exit(1);c.x-=65
		if(ui_button(rect(c.x,c.y,60,20),_t('cancel'),1)||ev.exit)modal_exit(0)
		if(ui_checkbox(rint(rect(b.x,b.y+20,b.w/2,16)),_t('go_to_card'),1,ms.act_go))ms.act_go^=1
		if(ui_radio(rect(cr.x+5,cr.y,80,16),_t('go_to_first')   ,ms.act_go,ms.act_gomode==0))ms.act_gomode=0;cr.y+=16
		if(ui_radio(rect(cr.x+5,cr.y,80,16),_t('prev_card'),ms.act_go,ms.act_gomode==1))ms.act_gomode=1;cr.y+=16
		if(ui_radio(rect(cr.x+5,cr.y,80,16),_t('next_card')    ,ms.act_go,ms.act_gomode==2))ms.act_gomode=2;cr.y+=16
		if(ui_radio(rect(cr.x+5,cr.y,80,16),_t('go_to_last')    ,ms.act_go,ms.act_gomode==3))ms.act_gomode=3;cr.y+=16
		if(ui_radio(rect(cr.x+5,cr.y,80,16),_t('go_back')    ,ms.act_go,ms.act_gomode==4))ms.act_gomode=4;cr.y+=16
		if(ui_radio(rect(cr.x+5,cr.y,45,16),_t('select')+':'   ,ms.act_go,ms.act_gomode==5))ms.act_gomode=5
		if(ms.act_go&&ms.act_gomode==5){
			...
			if(ui_button(rect(b.x+b.w-60,cr.y,60,20),_t('choose'),ms.act_go&&ms.act_gomode==5))modal_push('pick_card')
		}cr.y+=26;
		if(ui_checkbox(rect(cr.x,cr.y,80,16),_t('play')+' '+_t('sounds'),1,ms.act_sound))ms.act_sound^=1
		if(ms.act_go){
			if(ui_checkbox(rint(rect(b.x+b.w/2,b.y+20,b.w/2-19,16)),_t('with_transition'),1,ms.act_trans))ms.act_trans^=1
			if(ms.act_trans){
		...
		if(ms.act_sound){
			...
			if(ui_button(rect(b.x+b.w-60,cr.y,60,20),_t('choose'),ms.act_sound)){
		...
		draw_textc(rect(b.x,b.y,b.w,16),_t('select_card_any'),FONT_BODY,1)
		const c=rint(rect(b.x+(b.w-60-5-60-5-60)/2,b.y+b.h-20))
		if(ui_button(rect(c.x,c.y,60,20),_t('previous'),1)||ev.dir=='left')n_go([lms('Prev')],deck);c.x+=65
		if(ui_button(rect(c.x,c.y,60,20),_t('select'),1)){
		...
		if(ui_button(rect(c.x,c.y,60,20),_t('next'),1)||ev.dir=='right')n_go([lms('Next')],deck)
	}
   	else if(ms.type=='autoswitch_settings'){
		const b=draw_modalbox(rect(280,250))
		draw_textc(rect(b.x,b.y-5,b.w,20),_t('auto_switch_settings_title'),FONT_MENU,1)
		
		// 当前选中卡片信息
		const currentCard = ifield(deck, 'card')
		const currentCardIndex = ln(ifield(currentCard, 'index'))
		const currentCardName = ls(ifield(currentCard, 'name'))
		
		// 设置起始和结束卡片按钮
		if(ui_button(rect(b.x+5,b.y+25,75,20),_t('set_as_start'),1)) {
			autoSwitchState.startCard = currentCardIndex
			// 如果起始卡片大于结束卡片，自动调整结束卡片
			if(autoSwitchState.startCard > autoSwitchState.endCard) {
				autoSwitchState.endCard = autoSwitchState.startCard
			}
		}
		
		if(ui_button(rect(b.x+85,b.y+25,75,20),_t('set_as_end'),1)) {
			autoSwitchState.endCard = currentCardIndex
			// 如果结束卡片小于起始卡片，自动调整起始卡片
			if(autoSwitchState.endCard < autoSwitchState.startCard) {
				autoSwitchState.startCard = autoSwitchState.endCard
			}
		}
		
		// 显示当前范围
		draw_text(rect(b.x+165,b.y+27,150,16),`${_t('range')}: ${autoSwitchState.startCard} - ${autoSwitchState.endCard}`,FONT_BODY,1)
		
		// 当前卡片间隔设置
		draw_text(rect(b.x+5,b.y+50,100,16),_t('current_card_interval'),FONT_BODY,1)
		...
		// 确保form2存在并设置当前值
		if(!ms.form2) {
			ms.form2 = fieldstr(lms(currentInterval.toString()))
		}
		
		// 实时检测输入框变化并立即应用（仅在非多选模式下）
		if(!autoSwitchState.multiSelect) {
			const prevInterval = ms.form2._prevValue || currentInterval.toString()
			const currentInputValue = ls(rtext_string(ms.form2.table))
			if(currentInputValue !== prevInterval) {
				const newInterval = parseInt(currentInputValue) || 100
				if(currentCardIndex >= autoSwitchState.startCard && currentCardIndex <= autoSwitchState.endCard) {
					const intervalIndex = currentCardIndex - autoSwitchState.startCard
					// 确保间隔数组有足够的元素
					while(autoSwitchState.intervals.length <= intervalIndex) {
						autoSwitchState.intervals.push(autoSwitchState.uniformInterval)
					}
					autoSwitchState.intervals[intervalIndex] = newInterval
				}
				ms.form2._prevValue = currentInputValue
			}
		}
		
		ui_field(rect(b.x+105,b.y+48,60,18),ms.form2)
		
		// 根据多选模式显示不同的按钮
		if(!autoSwitchState.multiSelect) {
			// 非多选模式：显示"应用到全部"按钮
			if(ui_button(rect(b.x+185,b.y+47,80,20),_t('apply_to_all'),1)) {
				const newInterval = parseInt(ls(rtext_string(ms.form2.table))) || 100
				const cardCount = Math.max(1, autoSwitchState.endCard - autoSwitchState.startCard + 1)
				
				// 确保间隔数组有足够的元素并设置所有值
				autoSwitchState.intervals = []
				for(let i = 0; i < cardCount; i++) {
					autoSwitchState.intervals.push(newInterval)
				}
				autoSwitchState.uniformInterval = newInterval
			}
		} else {
			// 多选模式：显示"应用到选中帧"按钮
			if(ui_button(rect(b.x+185,b.y+47,100,20),_t('apply_to_selected'),1)) {
				if(autoSwitchState.selectedCards.length > 0) {
					const newInterval = parseInt(ls(rtext_string(ms.form2.table))) || 100
					// 应用到所有选中的卡片
					for(let i = 0; i < autoSwitchState.selectedCards.length; i++) {
						const selectedCardIndex = autoSwitchState.selectedCards[i]
						if(selectedCardIndex >= autoSwitchState.startCard && selectedCardIndex <= autoSwitchState.endCard) {
							const intervalIndex = selectedCardIndex - autoSwitchState.startCard
							// 确保间隔数组有足够的元素
							while(autoSwitchState.intervals.length <= intervalIndex) {
								autoSwitchState.intervals.push(autoSwitchState.uniformInterval)
							}
		...
		// 卡片列表显示 - 多选模式和复制按钮移到左边
		// 多选模式复选框
		if(ui_checkbox(rect(b.x+5,b.y+75,100,16),_t('multi_select'),1,autoSwitchState.multiSelect)){
			autoSwitchState.multiSelect = !autoSwitchState.multiSelect
			// 关闭多选模式时清空选中列表
			if(!autoSwitchState.multiSelect){
				autoSwitchState.selectedCards = []
			}
		}
		
		// 复制并粘贴卡片按钮
		if(ui_button(rect(b.x+90,b.y+70,120,20),_t('copy_and_paste_card'),1)) {
			copyAndPasteCard()
		}
		
		draw_text(rect(b.x+215,b.y+77,b.w-220,16),_t('card_list'),FONT_BODY,1)
		const listArea = rect(b.x+5,b.y+95,b.w-10,130)
		draw_box(listArea,0,1)
		...
		// 卡片列表显示 - 完全按照原版文件菜单中卡片弹窗的方式实现
		const slot = 30  // 使用与原版相同的slot高度
		const ch = slot * count(deck.cards)  // 所有卡片的总高度
		let m = 0, curr = ifield(deck, 'card')  // 当前卡片
		
		if(!ms.grid) ms.grid = {scroll: 0, row: -1}
		
		const sbar = scrollbar(listArea, Math.max(0, ch - (listArea.h - 2)), 10, listArea.h, ms.grid.scroll, ch >= listArea.h, 0)
		const bb = sbar.size
		ms.grid.scroll = sbar.scroll
		bb.y++
		...
		// 遍历所有卡片，完全按照原版实现
		for(let z = 0; z < count(deck.cards); z++) {
			const c = rect(bb.x, bb.y + (z * slot) - ms.grid.scroll, bb.w, slot)
			const card = deck.cards.v[z]
			
			if(c.y > bb.y + bb.h || c.y + c.h < bb.y) continue  // 裁剪不可见项
			const cb = rclip(c, bb)  // 粗略裁剪
			
			// 布局：预览图 + 卡片名称 + 详细信息
			const p = rect(c.x + 2, c.y + 1, 40, 28)  // 预览图区域
			const t = rect(p.x + p.w + 5, p.y, bb.w - (2 + p.w + 5 + 5), font_h(FONT_MENU))  // 卡片名称区域
			const s = rect(t.x, t.y + t.h + 2, t.w, font_h(FONT_BODY))  // 详细信息区域
			
			// 点击切换到该卡片并更新当前卡片信息
			if(ev.md && dover(cb)) {
				const clickedCardIndex = ln(ifield(card, 'index'))
				
				if(autoSwitchState.multiSelect) {
					// 多选模式：切换选中状态
					const selectedIndex = autoSwitchState.selectedCards.indexOf(clickedCardIndex)
					if(selectedIndex >= 0) {
						// 取消选中
						autoSwitchState.selectedCards.splice(selectedIndex, 1)
					} else {
						// 添加到选中列表
						autoSwitchState.selectedCards.push(clickedCardIndex)
						// 排序选中列表
						autoSwitchState.selectedCards.sort((a, b) => a - b)
					}
				} else {
					// 非多选模式：切换到该卡片
					m = 1
					n_go([card], deck)  // 切换到点击的卡片
					curr = card
					ms.grid.row = z
					// 更新间隔输入框为当前卡片的间隔
					let clickedInterval = autoSwitchState.uniformInterval
					if(clickedCardIndex >= autoSwitchState.startCard && clickedCardIndex <= autoSwitchState.endCard) {
						const intervalIndex = clickedCardIndex - autoSwitchState.startCard
						clickedInterval = autoSwitchState.intervals[intervalIndex] || autoSwitchState.uniformInterval
					}
					ms.form2 = fieldstr(lms(clickedInterval.toString()))
				}
			}
			...
			// 显示卡片名称
			let cardName = ls(ifield(card, 'name'))
			const cardIndex = ln(ifield(card, 'index'))
			
			// 添加起始/结束标记
			if(cardIndex >= autoSwitchState.startCard && cardIndex <= autoSwitchState.endCard) {
				if(cardIndex === autoSwitchState.startCard && cardIndex === autoSwitchState.endCard) {
					cardName += ' [起始/结束]'
				} else if(cardIndex === autoSwitchState.startCard) {
					cardName += ' [起始]'
				} else if(cardIndex === autoSwitchState.endCard) {
					cardName += ' [结束]'
				}
			}
			
			draw_text_fit(t, cardName, FONT_MENU, col)
			
			// 显示详细信息（widget数量 + 间隔时间）
			let detailText = `${count(card.widgets)} 控件`//${count(card.widgets) == 1 ? '' : 's'}`
			if(cardIndex >= autoSwitchState.startCard && cardIndex <= autoSwitchState.endCard) {
				const intervalIndex = cardIndex - autoSwitchState.startCard
				// 确保间隔数组有足够的元素
				while(autoSwitchState.intervals.length <= intervalIndex) {
					autoSwitchState.intervals.push(autoSwitchState.uniformInterval)
				}
				const interval = autoSwitchState.intervals[intervalIndex] || autoSwitchState.uniformInterval
				detailText += ` | ${interval}ms`
			}
			draw_text_fit(s, detailText, FONT_BODY, col)
			...
			// 多选模式下的选中状态显示
			if(autoSwitchState.multiSelect) {
				const isSelected = autoSwitchState.selectedCards.indexOf(cardIndex) >= 0
				if(isSelected) {
					// 选中状态：绘制选中框
					draw_box(c, 0, 1)
					draw_box(rect(c.x+1, c.y+1, c.w-2, c.h-2), 0, 1)
				}
				// 多选模式下不高亮当前卡片
			} else {
				// 非多选模式：当前卡片高亮显示
			...
			// 绘制卡片缩略图
			draw_thumbnail(card, p)
			
			// 双击编辑间隔（仅在范围内）
			if(ev.dclick && over(cb) && cardIndex >= autoSwitchState.startCard && cardIndex <= autoSwitchState.endCard) {
				const intervalIndex = cardIndex - autoSwitchState.startCard
				const currentInterval = autoSwitchState.intervals[intervalIndex] || autoSwitchState.uniformInterval
				const newInterval = prompt(_t('enter_interval_prompt'), currentInterval)
				if(newInterval !== null && !isNaN(parseInt(newInterval))) {
					autoSwitchState.intervals[intervalIndex] = parseInt(newInterval)
					// 同时更新输入框
					ms.form2 = fieldstr(lms(parseInt(newInterval).toString()))
				}
			}
		}
		...
		// 如果有卡片切换，更新滚动位置
		if(m) {
			curr = ifield(deck, 'card')
			const y = (ln(ifield(curr, 'index')) * slot) - ms.grid.scroll
			if(y < 0) { ms.grid.scroll += y }
			if(y + slot >= bb.h) { ms.grid.scroll += (y + slot) - bb.h }
		}
		
		// 控制按钮 - 调整位置更靠近卡片列表
		const c = rect(b.x,b.y+200)
		if(ui_button(rect(c.x+5,c.y+30,95,20), autoSwitchState.enabled ? _t('stop') : _t('start_auto_switch'), 1)) {
			if(autoSwitchState.enabled) {
				stopAutoSwitch()
			} else {
				// 开始自动切换
		...
		// 开始录制按钮
		if(ui_button(rect(c.x+105,c.y+30,70,20),_t('start_recording'),1)) {
			// 开始录制并自动播放
		...
		if(ui_button(rect(b.x+b.w-60,c.y+30,55,20),_t('ok'),1)||ev.exit) {
			// 关闭模态框
			modal_exit(0)
		}
	}
...
n_open=([type,hint,prompt])=>{
	modal_enter('open_lil');let t=type?ls(type):'',r=lms('');ms.filter='';ms.prompt=''
	...
	if(image_is(r)&&hint&&(ls(hint)=='frames'||ls(hint)=='gray_frames'))r=readgif([],ls(hint)),ms.filter='.gif'
	if(prompt)ms.prompt=ls(prompt)
	ms.verb=t=='array'?lms(t): hint?ls(hint):'';return r
}
n_save=([x,s])=>{
	modal_enter('save_lil');x=x||NIL;ms.path_suffix=array_is(x)&&s?ls(s):''
	if(array_is(x)                                      )ms.desc=_t('save_binary')        ,ms.text=fieldstr(lms('untitled'+ms.path_suffix))
	if(sound_is(x)                                      )ms.desc=_t('save_wav')    ,ms.text=fieldstr(lms('sound.wav'))
	if(deck_is(x)                                       )ms.desc=_t('save_deck_html'),ms.text=fieldstr(lms('untitled.deck'))
	if(image_is(x)||lid(x)||(lil(x)&&x.v.some(image_is)))ms.desc=_t('save_gif')    ,ms.text=fieldstr(lms('image.gif'))
...
const LCAPS=[
	...
	[KS('ArrowLeft','',1),KS('ArrowDown','',1),KS('ArrowUp','',1),KS('ArrowRight','',1),KS('alt','alt',1),KS(' ',' ',5),KS('alt','alt',1),KS(-2,'',2),KS(-1,_t('ok'),2)],
]
const UCAPS=[
...
soft_keyboard=r=>{
	...
	for(let row=0;row<LCAPS.length;row++){
		...
		keys.forEach((k,i,arr)=>{
			...
			else                      draw_textc(b,e?'运行':k.l,FONT_MENU,1)
...
bg_paste=(image,fit,pos)=>{
	...
	if(bg_has_sel()){bg_scoop_selection(),dr.limbo=image,dr.limbo_dither=0}
		...
		settool('select'),dr.sel_start=rect()
		if(pos){dr.sel_here=rect(pos.x,pos.y,s.x,s.y)}
		else{dr.sel_here=rcenter(con_view_dim(),s)}
		dr.limbo=image,dr.limbo_dither=0
	}
}
...
const tcellw=22, tcellh=19, tgap=1, toolsize=rect(tcellw*2+1,tcellh*19+tgap+1), tfb=image_make(toolsize), tid=new ImageData(toolsize.x,toolsize.y)
...
toolbars=_=>{
	...
	toolbar(q('#rtools'),q('#rrender'),(pos,dn)=>{
		...
		if(modebtn(pos,dn,rect(0,0     ,tcellw*2+1,tcellh+1),_t('stroke'),dr.pickfill==0))dr.pickfill=0
		if(modebtn(pos,dn,rect(0,tcellh,tcellw*2+1,tcellh+1),_t('fill')  ,dr.pickfill==1))dr.pickfill=1
		
		// 显示调色板
		if(dr.color){
			for(let z=0;z<16 ;z++){
				const colorIndex = (z>=2?31:0)+z
				palbtn(pos,dn,rect(0,(2*tcellh)+z*tcellh,2*tcellw+1,tcellh+1),colorIndex)
			}
		}
		else{
			for(let z=0;z<4*8;z++){
				const colorIndex = patorder[z]
				palbtn(pos,dn,rect((z%2)*tcellw,(2*tcellh)+(0|(z/2))*tcellh+(z>=28?tgap:0),tcellw+1,tcellh+1),colorIndex)
			}
		}
		
		// 添加编辑颜色按钮
		const editBtnY = dr.color ? (2*tcellh)+16*tcellh : (2*tcellh)+16*tcellh
		if(modebtn(pos,dn,rect(0,editBtnY,tcellw*2+1,tcellh+1),_t('edit_color'),0)){
			const currentPattern = dr.pickfill?dr.fill:dr.pattern
			if(currentPattern >= 32 && currentPattern <= 47){
				show_color_picker_for_pattern(currentPattern)
			}
		}
	})
}
...
setscript=x=>{
	...
	sc.status=p?_t('no_script_template_filled'):'',sc.f=fieldstr(v),wid.active=0
}
...
close_script=next=>{
	...
	try{const text=ls(rtext_string(sc.f.table));parse(text),script_save(lms(text)),finish_script()}
		...
		ms.message=lms(_t('script_error_discard',{error:e.x})),ms.verb=_tlms('discard')
	}
}
script_editor=r=>{
	...
	if(sc.status.length){draw_text_fit(rect(r.x+3,r.y+r.h-mh+3,r.w,mh-6),sc.status,FONT_BODY,1);}
		...
		let stat='';if(in_layer()&&wid.infield){
			...
			if(a!=b&&a<x.layout.length){
				...
				stat=`已选择 ${l} 行${l==1?'':''}，${c} 个字符${c==1?'':''}`
			}else{const p=field_position(x,min(a,b));stat=`第 ${p.x} 行，第 ${p.y} 列`}
		}
		const l=font_textsize(FONT_BODY,stat);draw_text(rect(3,r.y+r.h-mh+3,l.x,l.y),stat,FONT_BODY,1)
		stat=`${sc.target.n} '${ls(ifield(sc.target,'name'))}' 的脚本${sc.others.length?` 和其他 ${sc.others.length} 个`:''}`
...
interpret=_=>{
	...
	let quota=FRAME_QUOTA;while(1){
		while(nomodal()&&running()&&sleep_frames==0&&sleep_play==0&&quota>0){runop(),quota--,mark_dirty(1)}frame=context
...
text_edit_menu=_=>{
	...
	if(menu_item(_t('undo'),wid.hist_cursor>0              ,'z'))field_undo()
	if(menu_item(_t('redo'),wid.hist_cursor<wid.hist.length,'Z'))field_redo()
	menu_separator()
	if(menu_item(_t('cut'),selection,'x',menucut)){}
	if(menu_item(_t('copy'),selection,'c',menucopy)){}
	if(rich&&menu_item(_t('copy_rich_text'),selection,0,menucopyrich)){}
	if(menu_item(_t('paste'),wid.fv!=null,'v',menupaste)){}
	if(menu_item(_t('clear'),wid.fv!=null))wid.cursor=rect(0,RTEXT_END),field_keys('Delete',0)
	if(!enable_touch&&!kc.on){menu_separator();if(menu_item(_t('soft_keyboard'),wid.fv!=null))keycaps_force_enter()}
	// 调色板功能 - 只在编辑文本时显示
	if(wid.fv!=null){menu_separator();if(menu_item(_t('color_picker'),wid.fv!=null))show_color_picker()}
	// 暂时隐藏中文输入选项，问题待解决
	// if(menu_check('中文输入',wid.fv!=null,window.chineseInputEnabled||false))window.chineseInputEnabled^=1
	menu_separator()
	if(menu_item(_t('select_all'),wid.fv!=null,'a'))wid.cursor=rect(0,RTEXT_END)
}
all_menus=_=>{
	const blocked=running()||msg.overshoot
	const canlisten=!blocked&&(ms.type=='listen'||ms.type==null)
	// 确保 i18n 已初始化
	if(typeof i18n !== 'undefined' && i18n && typeof i18n.init === 'function' && !i18n.currentLang){
		i18n.init();
	}
	menu_bar('WPaint',canlisten&&!kc.on)
	if(menu_item(_t('about'),1))modal_enter('about')
	menu_separator()
	// 语言切换
	if(typeof i18n !== 'undefined' && i18n && !window.CHINESE_ONLY){
		// 确保 i18n 已初始化
		if(!i18n.currentLang && typeof i18n.init === 'function'){
			i18n.init();
		}
		if(menu_check(_t('chinese'),1,i18n.currentLang=='zh')){i18n.setLang('zh');resize();if(msg)msg.pending_view=1}
		if(menu_check(_t('english'),1,i18n.currentLang=='en')){i18n.setLang('en');resize();if(msg)msg.pending_view=1}
		menu_separator()
	}
	if(menu_check(_t('listener'),canlisten,ms.type=='listen','l')){if(ms.type!='listen'){modal_enter('listen')}else{modal_exit(0)}}
	menu_separator()
	menu_check(_t('fullscreen'),1,is_fullscreen(),null,toggle_fullscreen)
	if(menu_check(_t('touch_input'),1,enable_touch)){enable_touch^=1,set_touch=1;if(!enable_touch)kc.on=0}
	if(menu_check(_t('script_analyzer'),1,profiler))profiler^=1
	if(menu_check(_t('toolbar'),tzoom>0,toolbars_enable))toolbars_enable^=1,resize()
	if(blocked){
		menu_bar(_t('menu_script'),1)
		if(menu_item(_t('stop'),1)){msg.pending_halt=1;if(ms.type!='query'&&ms.type!='listen'){if(ms.type!=null){modal_exit(0)}setmode('object')}}
		menu_bar(_t('menu_edit'),(ms.type=='input'||ms.type=='save')&&wid.fv)
		text_edit_menu()
		return
	}
	menu_bar(_t('menu_file'),(ms.type==null||ms.type=='recording')&&(!kc.on||uimode=='script'))
	if(uimode=='script'){
		if(menu_item(_t('close_script'),1))close_script()
		if(menu_item(_t('save_script'),1,'s')){
			try{const text=ls(rtext_string(sc.f.table));parse(text),script_save(lms(text));sc.status=_t('saved')}
			catch(e){sc.status=`Error: ${e.x}`,wid.cursor=rect(e.i,e.i)}
		}
		menu_separator()
		menu_item(_t('import_script'),1,0,_=>open_text('.lil,.txt',text=>{field_exit(),sc.f=fieldstr(lms(text))}))
		if(menu_item(_t('export_script_menu'),1))modal_enter('export_script')
		menu_separator()
		if(menu_item(_t('go_to_deck'),!deck_is(sc.target)           ))close_script(deck)
		const container=con()
		if(menu_item(_t(prototype_is(container)?'go_to_prototype':'go_to_card'),sc.target!=container))close_script(container)
		if(menu_check(_t('perspective_mode'),!kc.on,sc.xray))sc.xray^=1
	}
	else if(ms.type=='recording'){
		menu_item(_t('import_sound'),1,0,_=>open_file('audio/*',load_sound))
		menu_separator();
		if(menu_item(_t('close_sound'),1))modal_exit(0)
	}
	else{
		if(menu_item(_t('new_deck'),1)){
			if(dirty){
				modal_enter('confirm_new')
				ms.message=_tlms('unsaved_changes')
				ms.verb=_tlms('discard')
			}else{load_deck(deck_read(''))}
		}
		if(menu_item(_t('new_card'),1)){const c=deck_add(deck,lms('card')), n=ln(ifield(ifield(deck,'card'),'index'));iwrite(c,lms('index'),lmn(n+1)),n_go([c],deck)}
		menu_separator()
		menu_item(_t('open_menu'),1,0,_=>open_text('.html,.deck',text=>{load_deck(deck_read(text))}))
		if(menu_item(_t('save_as'),1))modal_enter('save_deck')
		menu_separator()
		menu_item(_t('import_image'),1,0,_=>{open_file('image/*',load_image)})
		if(menu_item(_t('export_image_menu'),1))modal_enter('export_image')
		if(menu_item(_t('save_gif_menu'),1)){
			// 拼接所有卡片的 wiggler 装置 frames 并保存为 GIF（排除 card1）
			const save_all_wiggler_gif=_=>{
				try{
					if(!deck||!deck.cards)return
					
					// 获取所有卡片的 frames
					let all_frames=[], all_delays=[]
					
					// 遍历所有卡片，从索引 1 开始（排除 card1）
					for(let i=1;i<count(deck.cards);i++){
						const card=deck.cards.v[i]
						if(!card||!card.widgets)continue
						
						const target_widget=dget(card.widgets,lms('target'))
						if(!target_widget||!contraption_is(target_widget))continue
						
						const def=ifield(target_widget,'def')
						const def_name=ls(ifield(def,'name'))
						if(def_name!='wiggler')continue
						
						// 获取当前卡片 wiggler 的 frames
						let card_frames=[]
						
						// 方法1: 尝试通过 frames 属性直接获取
						let frames_val=null
						try{
							frames_val=ifield(target_widget,lms('frames'))
							if(lil(frames_val)&&frames_val.v.length>0){
								card_frames=ll(frames_val).filter(image_is)
							}
						}catch(e){
							console.error(`Error getting frames from card ${i}:`,e)
						}
						
						// 方法2: 通过 get_value 方法获取
						if(card_frames.length==0){
							try{
								const value=fire_attr_sync(target_widget,'get_value',null)
								if(lid(value)){
									const frames_val=dget(value,lms('frames'))
									if(lil(frames_val))card_frames=ll(frames_val).filter(image_is)
								}
							}catch(e){
								console.error(`Error getting value from card ${i}:`,e)
							}
						}
						
						// 方法3: 尝试通过 get_frames 方法直接获取
						if(card_frames.length==0){
							try{
								const frames_val=fire_attr_sync(target_widget,'get_frames',null)
								if(lil(frames_val))card_frames=ll(frames_val).filter(image_is)
							}catch(e){
								console.error(`Error getting frames via get_frames from card ${i}:`,e)
							}
						}
						
						// 将当前卡片的 frames 添加到总列表
						if(card_frames.length>0){
							all_frames=all_frames.concat(card_frames)
							// 每帧延迟 10 个单位（约 0.1 秒）
							all_delays=all_delays.concat(card_frames.map(_=>10))
						}
					}
					
					if(all_frames.length==0)return
					
					// 显示确认对话框，询问是否需要透明背景
					modal_enter('confirm_transparent_bg')
					ms.message=_tlms('select_background_type')
					ms.verb=_tlms('transparent_background')
					ms.cancel_text=_tlms('opaque_background')
					// 保存待处理的 GIF 数据
					ms.pending_gif_frames=all_frames
					ms.pending_gif_delays=all_delays
				}catch(e){
					console.error('Error saving all wiggler GIF:',e)
				}
			}
			save_all_wiggler_gif()
		}
		menu_separator()
		if(menu_item(_t('clear_volatile'),1)){deck_purge(deck),msg.next_view=1}
		menu_separator()
		if(menu_item(_t('cards_menu')     ,1,'C'))modal_enter('cards')
		if(menu_item(_t('sounds_menu')    ,1,'S'))modal_enter('sounds')
		if(menu_item(_t('prototypes_menu'),1,'T'))modal_enter('contraptions')
		if(menu_item(_t('resources') ,1,   ))modal_enter('resources')
		if(menu_item(_t('properties_menu'),1,   ))modal_enter('deck_props')
if(menu_item(_t('resize'),1))modal_enter('resize_deck')
	}
	if(ms.type==null||wid.gv||wid.fv){
		menu_bar(_t('menu_edit'),wid.gv||wid.fv||(ms.type==null&&uimode=='interact')||uimode=='draw'||uimode=='object')
		if(wid.gv){
			const mutable=!wid.g.locked&&ms.type==null
			if(menu_item(_t('undo'),wid.hist_cursor>0              ,'z'))grid_undo()
			if(menu_item(_t('redo'),wid.hist_cursor<wid.hist.length,'Z'))grid_redo()
			menu_separator()
			if(menu_item(_t('copy_table'),1,'c',menucopy)){}
			if(menu_item(_t('paste_table'),mutable,'v',menupaste)){}
			menu_separator()
			if(menu_item(_t('delete_row'),mutable&&wid.gv.row!=-1))grid_deleterow()
			if(menu_item(_t('add_row'),mutable))grid_insertrow()
			if(menu_item(_t('query_menu'),ms.type==null,'u'))modal_enter('query')
		}
		if(wid.fv)text_edit_menu()
		if(ms.type==null&&uimode=='interact'){
			if(menu_item(_t('undo'),has_undo(),'z'))undo()
			if(menu_item(_t('redo'),has_redo(),'Z'))redo()
			menu_separator()
			if(menu_item(_t('paste'),1,'v',menupaste)){}
		}
		if(ms.type==null&&uimode=='draw'){
			const sel=bg_has_sel()||bg_has_lasso()
			if(menu_item(_t('undo'),(!sel)&&has_undo(),'z'))undo()
			if(menu_item(_t('redo'),(!sel)&&has_redo(),'Z'))redo()
			menu_separator()
			if(menu_item(_t('cut_image'),sel,0,menucut)){}
			if(menu_item(_t('copy_image'),sel,0,menucopy)){}
			if(menu_item(_t('paste'),1,'v',menupaste)){}
			if(menu_item(_t('clear'),1)){const t=dr.tool;if(!sel){settool('select'),dr.sel_here=rcopy(con_dim())}bg_delete_selection(),settool(t)}
			menu_separator()
			if(menu_item(_t('select_all'),1,'a')){settool('select'),dr.sel_here=rcopy(con_dim())}
			if(menu_item(_t('tight_selection'),sel,'g'))bg_tighten()
			if(menu_item(_t('add_outline'),sel))bg_outline()
			if(menu_item(_t('resize_to_original'),sel&&dr.tool=='select',0)){bg_scoop_selection();const s=dr.limbo.size;dr.sel_here.w=s.x,dr.sel_here.h=s.y}
			if(menu_item(_t(prototype_is(con())?'resize_to_prototype':'resize_to_card'),sel&&dr.tool=='select',0)){bg_scoop_selection(),dr.sel_here=con_dim()}
			menu_separator()
			if(menu_item(_t('invert'),sel&&!dr.limbo_dither,'i')){
			...
			if(menu_item(_t('rotate_right'),sel,'.')){
				const s=rect(dr.sel_here.w,dr.sel_here.h)
				if(bg_has_sel())bg_scoop_selection()
			...
			if(dr.limbo_dither&&sel){
				menu_separator()
				if(menu_item(_t('brighten'),dr.dither_threshold>-2.0))dr.dither_threshold-=.1
				if(menu_item(_t('darken'),dr.dither_threshold< 2.0))dr.dither_threshold+=.1
			}
		}
		if(ms.type==null&&uimode=='object'){
			...
			if(menu_item(_t('redo'),has_redo(),'Z'))redo()
			menu_separator()
			if(menu_item(_t('cut_widgets'),ob.sel.length,'x',menucut)){}
			if(menu_item(_t('copy_widgets'),ob.sel.length,'c',menucopy)){}
			if(menu_item(_t('copy_image'),ob.sel.length==1,0,copywidgetimg)){}
			if(menu_item(_t('paste'),1,'v',menupaste)){}
			menu_separator()
			if(menu_item(_t('paste_as_new_canvas'),1,0,pasteascanvas)){}
			if(menu_item(_t('paste_into_canvas'),ob.sel.length==1&&canvas_is(ob.sel[0]),0,pasteintocanvas)){}
			menu_separator()
			if(menu_item(_t('select_all'),1,'a'))ob.sel=con_wids().v.slice(0)
			if(menu_item(_t('move_to_front'),ob.sel.length))ob_order(),ob.sel                   .map(w=>iwrite(w,lms('index'),lmn(RTEXT_END))),mark_dirty()
			if(menu_item(_t('move_up')      ,ob.sel.length))ob_move_up()
			if(menu_item(_t('move_down')    ,ob.sel.length))ob_move_dn()
			if(menu_item(_t('move_to_back') ,ob.sel.length))ob_order(),ob.sel.slice(0).reverse().map(w=>iwrite(w,lms('index'),ZERO          )),mark_dirty()
		}
		if(wid.fv&&wid.f){
			const selection=wid.fv!=null&&wid.cursor.x!=wid.cursor.y
			menu_bar(_t('menu_text'),selection&&wid.f.style!='plain')
			if(wid.f.style=='rich'){
				if(menu_item(_t('title')    ,selection))field_stylespan(lms('menu'),lms(''))
				if(menu_item(_t('body')       ,selection))field_stylespan(lms(''    ),lms(''))
				if(menu_item(_t('monospace'),selection))field_stylespan(lms('mono'),lms(''))
				if(menu_item(_t('link_menu')    ,selection))modal_push('link')
			}
			else if(wid.f.style=='code'){
				if(menu_item(_t('indent')        ,1    ))field_indent(1)
				if(menu_item(_t('unindent')      ,1    ))field_indent(0)
				if(menu_item(_t('toggle_comment'),1,'/'))field_comment()
			}
			if(wid.f&&wid.f.style!='code'){
				if(menu_item(_t('font_menu'),wid.f.style!='plain'))modal_push('fonts')
			}
		}
	}
	if(ms.type=='recording'&&!wid.fv){
		menu_bar(_t('menu_edit'),au.mode=='stopped')
		if(menu_item(_t('undo'),au.hist_cursor>0             ,'z'))sound_undo()
		if(menu_item(_t('redo'),au.hist_cursor<au.hist.length,'Z'))sound_redo()
		menu_separator()
		if(menu_item(_t('cut_sound')  ,1,'x',menucut)){}
		if(menu_item(_t('copy_sound') ,1,'c',menucopy)){}
		if(menu_item(_t('paste_sound'),1,'v',menupaste)){}
		if(menu_item(_t('clear'),1,0))sound_delete()
		menu_separator()
		if(menu_item(_t('select_all'),1,'a'))au.head=0,au.sel=rect(0,au.target.data.length-1)
	}
	// // 画布菜单 - 只在交互模式下显示 无效代码，注释掉了
	// if(uimode=='interact'&&ms.type==null&&!kc.on){
	// 	menu_bar('画布',1)
	// 	if(menu_item('撤销',has_undo(),'z'))undo()
	// 	if(menu_item('重做',has_redo(),'Z'))redo()
	// }
	if((uimode=='interact'||uimode=='draw'||uimode=='object')&&card_is(con())){
		menu_bar(_t('card'),ms.type==null&&!kc.on)
		if(menu_item(_t('go_to_first')   ,1))n_go([lms('First')],deck)
		if(menu_item(_t('go_to_previous'),1))n_go([lms('Prev' )],deck)
		if(menu_item(_t('go_to_next')    ,1))n_go([lms('Next' )],deck)
		if(menu_item(_t('go_to_last')    ,1))n_go([lms('Last' )],deck)
		if(menu_item(_t('go_back'),deck.history.length>1))n_go([lms('Back')],deck)
		menu_separator()
		if(menu_item(_t('cut_card'),1,0,cutcard)){}
		if(menu_item(_t('copy_card'),1,0,copycard)){}
		if(menu_item(_t('copy_and_paste_card'),1)){copyAndPasteCard()}
		menu_separator()
		if(menu_item(_t('auto_switch_settings'),1))modal_enter('autoswitch_settings')
		if(menu_item(_t(autoSwitchState.enabled ? 'stop_auto_switch' : 'start_auto_switch'),1)){
			if(autoSwitchState.enabled) {
				stopAutoSwitch()
			} else {
				startAutoSwitch()
		...
		menu_separator()
		if(menu_item(_t('script')    ,1))setscript(con())
		if(menu_item(_t('properties'),1))modal_enter('card_props')

	}
	if((uimode=='interact'||uimode=='draw'||uimode=='object')&&prototype_is(con())){
		menu_bar(_t('prototype'),ms.type==null&&!kc.on)
		const def=con(), defs=deck.contraptions
		if(menu_item(_t('close'),1))con_set(null)
		if(menu_item(_t('go_to_previous_menu'),count(defs)>1)){ev.dir='left' ,tracking(),ev.dir=0}
		if(menu_item(_t('go_to_next_menu')    ,count(defs)>1)){ev.dir='right',tracking(),ev.dir=0}
		menu_separator()
		if(menu_item(_t('script')    ,1))setscript(con())
		if(menu_item(_t('properties'),1))modal_enter('prototype_props')
		if(menu_item(_t('attributes'),1))modal_enter('prototype_attrs')
		if(menu_check(_t('show_margins'),1,ob.show_margins))ob.show_margins^=1
		menu_separator()
		let r=lb(ifield(def,'resizable'))
		if(menu_check(_t('resizable'),1,r,0)){r^=1,iwrite(def,lms('resizable'),lmn(r)),mark_dirty()}
	}
	if(uimode=='interact'||uimode=='draw'||uimode=='object'){
		menu_bar(_t('menu_tool'),ms.type==null&&!kc.on)
		if(menu_check(_t('interact'),1,uimode=='interact',0))setmode('interact')
		if(menu_check(_t('widget') ,1,uimode=='object'  ,0))setmode('object')
		menu_separator()
		if(menu_check(_t('select_tool')     ,1,uimode=='draw'&&dr.tool=='select'     ))settool('select'     )
		if(menu_check(_t('lasso')      ,1,uimode=='draw'&&dr.tool=='lasso'      ))settool('lasso'      )
		if(menu_check(_t('pencil')     ,1,uimode=='draw'&&dr.tool=='pencil'     ))settool('pencil'     )
		if(menu_check(_t('line')       ,1,uimode=='draw'&&dr.tool=='line'       ))settool('line'       )
		if(menu_check(_t('fill')      ,1,uimode=='draw'&&dr.tool=='fill'       ))settool('fill'       )
		if(menu_check(_t('rectangle')        ,1,uimode=='draw'&&dr.tool=='rect'       ))settool('rect'       )
		if(menu_check(_t('filled_rectangle') ,1,uimode=='draw'&&dr.tool=='fillrect'   ))settool('fillrect'   )
		if(menu_check(_t('ellipse')       ,1,uimode=='draw'&&dr.tool=='ellipse'    ))settool('ellipse'    )
		if(menu_check(_t('filled_ellipse'),1,uimode=='draw'&&dr.tool=='fillellipse'))settool('fillellipse')
		if(menu_check(_t('polygon')    ,1,uimode=='draw'&&dr.tool=='poly'       ))settool('poly'       )
	}
	if(uimode=='draw'||uimode=='object'){
		menu_bar(_t('menu_view'),ms.type==null&&!kc.on)
		if(menu_check(_t('show_widgets')         ,1,dr.show_widgets))dr.show_widgets^=1
		if(menu_check(_t('show_widget_boundaries')   ,1,ob.show_bounds ))ob.show_bounds ^=1
		if(menu_check(_t('show_widget_names')    ,1,ob.show_names  ))ob.show_names  ^=1
		if(menu_check(_t('show_cursor_info')     ,1,ob.show_cursor ))ob.show_cursor ^=1
		if(menu_check(_t('show_alignment_guides'),1,ob.show_guides ))ob.show_guides ^=1
		menu_separator()
		if(menu_check(_t('show_grid_overlay'),1,dr.show_grid))dr.show_grid^=1
		if(menu_check(_t('snap_to_grid')     ,1,dr.snap     ))dr.snap     ^=1
		if(menu_item(_t('grid_and_scale'),1))modal_enter('grid')
		menu_separator()
		if(menu_check(_t('show_animation')   ,1,dr.show_anim ))dr.show_anim ^=1
		if(menu_check(_t('transparency_mask'),1,dr.trans_mask))dr.trans_mask^=1
		if(menu_check(_t('fatbits')         ,1,dr.fatbits   )){
	...
	if(uimode=='draw'){
		menu_bar(_t('menu_style'),ms.type==null&&!kc.on)
		if(menu_item(_t('stroke_menu'),1))modal_enter('pattern')
		if(menu_item(_t('fill_menu')  ,1))modal_enter('fill'   )
		if(menu_item(_t('brush_menu') ,1))modal_enter('brush'  )
		menu_separator()
		// 这些选项没有对应的翻译键，暂时保留中文
		if(menu_check(_t('color')       ,1,dr.color))dr.color^=1
		if(menu_check(_t('transparency'),1,dr.trans))dr.trans^=1
		if(menu_check(_t('underlay')  ,1,dr.under))dr.under^=1
	}
	if(uimode=='object'){
		menu_bar(_t('widget'),ms.type==null)
		if(menu_item(_t('new_button')     ,1))ob_create([lmd([lms('type')],[lms('button')])])
		if(menu_item(_t('new_field')      ,1))ob_create([lmd([lms('type')],[lms('field' )])])
		if(menu_item(_t('new_slider')     ,1))ob_create([lmd([lms('type')],[lms('slider')])])
		if(menu_item(_t('new_canvas')     ,1))ob_create([lmd([lms('type')],[lms('canvas')])])
		if(menu_item(_t('new_grid')       ,1))ob_create([lmd([lms('type')],[lms('grid'  )])])
		if(card_is(con())&&menu_item(_t('new_contraption_menu'),1))modal_enter('pick_contraption')
		menu_separator()
		if(menu_item(_t('sort')   ,count(ifield(con(),'widgets'))))modal_enter('orderwids')
		menu_separator()
		...
		if(menu_check(_t('lock')          ,ob.sel.length,ob.sel.length&&al))ob_edit_prop('locked'  ,lmn(!al))
		if(menu_check(_t('animate')        ,ob.sel.length,ob.sel.length&&aa))ob_edit_prop('animated',lmn(!aa))
		if(menu_check(_t('volatile')        ,ob.sel.length,ob.sel.length&&av))ob_edit_prop('volatile',lmn(!av))
		menu_separator()
		if(menu_check(_t('show_solid')      ,ob.sel.length,ob.sel.length&&as))ob_edit_prop('show',lms('solid'      ))
		if(menu_check(_t('show_transparent'),ob.sel.length,ob.sel.length&&at))ob_edit_prop('show',lms('transparent'))
		if(menu_check(_t('show_invert')   ,ob.sel.length,ob.sel.length&&ai))ob_edit_prop('show',lms('invert'     ))
		if(menu_check(_t('show_none')       ,ob.sel.length,ob.sel.length&&an))ob_edit_prop('show',lms('none'       ))
		menu_separator()
		if(menu_item(_t('font_menu')  ,ob.sel.length))modal_enter('fonts')
		if(menu_item(_t('script'),ob.sel.length)){
			if(ob.sel.reduce((m,v)=>m&&ob.sel[0].script==v.script,1)){setscript(ob.sel)}else{
				modal_enter('multiscript')
					ms.message=_tlms('different_scripts_edit_together')
					ms.verb=_tlms('edit')
			}
		}
		if(menu_item(_t('properties'),ob.sel.length==1)||(ob.sel.length==1&&ev.action&&ms.type==null))object_properties(ob.sel[0])
	}
	if(ms.type=='listen'){
		menu_bar(_t('listener'),1)
		if(menu_item(_t('clear_history'),1))li.hist=[],li.scroll=0
		if(menu_item(_t('clear_locals') ,1))li.vars=new Map()
		menu_separator()
		if(menu_item(_t('show_locals'),1)){
			const loc=lmd();for(let k of li.vars.keys())dset(loc,lms(k),li.vars.get(k))
			listen_show(ALIGN.right,0,loc)
		}
		menu_separator()
		if(menu_item(_t('evaluate'),rtext_len(ms.text.table)))listener_eval()
	}
	menu_bar(_t('menu_help'),1)
	if(menu_item('Wpaint 网站...'  ,1))n_go([lms('https://wpaint.fun'          )],deck)
	if(menu_item('Decker 网站...'  ,1))n_go([lms('http://beyondloom.com/decker/index.html'          )],deck)
	if(menu_item('Decker 社区...',1))n_go([lms('https://internet-janitor.itch.io/decker/community')],deck)
	if(menu_item('Decker 参考...',1))n_go([lms('http://beyondloom.com/decker/decker.html'         )],deck)
	if(menu_item('Lil 参考...'   ,1))n_go([lms('http://beyondloom.com/decker/lil.html'            )],deck)
}
...
load_deck=d=>{
	// console.log('load_deck: 开始加载deck')
	// console.log('load_deck: deck.patterns存在?', !!d.patterns)
	// if(d.patterns && d.patterns.pal && d.patterns.pal.pix) {
	// 	console.log('load_deck: patterns颜色值:', Array.from({length:16}, (_,i)=>pal_col_get(d.patterns.pal.pix,i).toString(16)))
	// }
	deck=d, 
	dirty=0, 
	wid.active=-1, 
	...
	dr=draw_state(), 
	con_set(null)
	FONT_BODY=dget(deck.fonts,lms('body')),
	FONT_MENU=dget(deck.fonts,lms('menu')),
	FONT_MONO=dget(deck.fonts,lms('mono'))
	fb=image_make(deck.size),
	context=frame=draw_frame(fb),
	validate_modules(),
	setmode('interact'),
	...
	// 加载自动切换配置
	const load_autoswitch = (time1=0)=>{
		if(time1>10)return
		try{
			//console.log('Loading deck with properties:', Object.keys(deck))
			if (deck.hasOwnProperty('autoswitch_uniformInterval')) {
				autoSwitchState.uniformInterval = ln(deck['autoswitch_uniformInterval'])
				console.log('Loaded uniformInterval:', autoSwitchState.uniformInterval)
			}
			if (deck.hasOwnProperty('autoswitch_intervals')) {
				try {
					// 处理Lil列表对象
					const intervalsData = deck['autoswitch_intervals']
					console.log('Raw intervals data:', intervalsData)
					if (intervalsData && intervalsData.v && Array.isArray(intervalsData.v)) {
						// 从Lil列表对象中提取数值
						autoSwitchState.intervals = intervalsData.v.map(item => ln(item))
					} else if (typeof intervalsData === 'string') {
						// 如果是字符串，尝试JSON解析
						autoSwitchState.intervals = JSON.parse(intervalsData)
					} else {
						autoSwitchState.intervals = []
					}
					console.log('Loaded intervals:', autoSwitchState.intervals)
				} catch (e) {
					console.warn('Failed to parse autoswitch intervals:', e)
					autoSwitchState.intervals = []
				}
			}
			if (deck.hasOwnProperty('autoswitch_startCard')) {
				autoSwitchState.startCard = ln(deck['autoswitch_startCard'])
				console.log('Loaded startCard:', autoSwitchState.startCard)
			}
			if (deck.hasOwnProperty('autoswitch_endCard')) {
				autoSwitchState.endCard = ln(deck['autoswitch_endCard'])
				console.log('Loaded endCard:', autoSwitchState.endCard)
			}
			if (deck.hasOwnProperty('autoswitch_isEnabled')) {
			autoSwitchState.isEnabled = ln(deck['autoswitch_isEnabled']) === 1
			console.log('Loaded isEnabled:', autoSwitchState.isEnabled)
		}
			//console.log('他妈的，自动切换终于成功了')
		}
		catch(e)
		{
			console.log('第'+(time1+1)+'次加载失败')
			setTimeout(()=>{
				load_autoswitch(time1+1)
			},300)
		}
	}
	load_autoswitch()
...
tick=_=>{
	...
	if(uimode=='script'){const mh=(typeof window!='undefined'&&window.HIDE_NAV)?0:(3+font_h(FONT_MENU));if(!kc.on)script_editor(rect(0,mh,frame.size.x+1,frame.size.y-mh))}else{main_view()}
	modals(),gestures()
	...
	document.title=ls(ifield(deck,'name')) || _t('untitled_deck')
...
let id=null
let sync_first_call = true
sync=_=>{
	pick_palette(deck)
	if(sync_first_call) {
		//console.log('sync: 第一次调用，pick_palette后的COLORS:', COLORS.map(c=>(0xFFFFFF&c).toString(16)))
		sync_first_call = false
	}
...
mouse=(e,f)=>{
	// 检查调色板是否显示，如果显示则忽略所有鼠标事件
	const colorPickerModal = q('#colorPickerModal');
	if (colorPickerModal && colorPickerModal.style.display !== 'none') {
		// 检查事件目标是否在调色板模态框或Spectrum容器内
		if (colorPickerModal.contains(e.target)) {
			return; // 调色板内部，不处理
		}
		const spectrumContainer = q('.sp-container');
		if (spectrumContainer && spectrumContainer.contains(e.target)) {
			return; // Spectrum容器内，不处理
		}
		// 即使点击在外部，如果调色板显示，也忽略事件（防止触发底层canvas操作）
		return;
	}
...
const handleTouchStart = e => {
	// 语言切换弹窗打开时，放行 DOM 事件（避免 preventDefault 阻断按钮点击）
	const langModalOverlay = q('.lang-modal-overlay');
	if (langModalOverlay) return;

	// 检查调色板是否显示
	const colorPickerModal = q('#colorPickerModal');
	if (colorPickerModal && colorPickerModal.style.display !== 'none') {
		// 检查事件的原始目标是否在调色板模态框内部
		if (colorPickerModal.contains(e.target)) {
			// 如果事件源于调色板内部，则不执行任何Decker的逻辑，
			// 让事件正常传播给Spectrum处理。
			console.log("Touch event on color picker, letting it pass.");
			return;
		}

		// 检查事件的原始目标是否在Spectrum调色板容器内部
		const spectrumContainer = q('.sp-container');
		if (spectrumContainer && spectrumContainer.contains(e.target)) {
			// 如果事件源于Spectrum调色板容器内部，则不执行任何Decker的逻辑，
			// 让事件正常传播给Spectrum处理。
			console.log("Touch event on spectrum container, letting it pass.");
			return;
		}
		
		// 即使点击在外部，如果调色板显示，也忽略事件（防止触发底层canvas操作）
		console.log("Color picker is open, ignoring touch event on background");
		return;
	}

	// 如果调色板未显示，则执行Decker的常规操作
	e.preventDefault();
	touch(e, down);
};

// 其他监听器保持不变
q('body').addEventListener('touchstart', handleTouchStart, { passive: false });
q('body').addEventListener('touchend', e => {
	const langModalOverlay = q('.lang-modal-overlay');
	if (langModalOverlay) return;
	const colorPickerModal = q('#colorPickerModal');
	if (colorPickerModal && colorPickerModal.style.display !== 'none') return;
	touch({ targetTouches: e.changedTouches }, up);
})
q('body').addEventListener('touchmove', e => {
	const langModalOverlay = q('.lang-modal-overlay');
	if (langModalOverlay) return;
	const colorPickerModal = q('#colorPickerModal');
	if (colorPickerModal && colorPickerModal.style.display !== 'none') return;
...
// 添加input事件监听器来处理输入法输入
let hiddenInput = null;
let isInputMethodActive = false;
// 中文输入开关，默认关闭
window.chineseInputEnabled = false;
...
function createHiddenInput() {
	if (!hiddenInput) {
		...
		hiddenInput.addEventListener('input', e => {
			if (wid.infield && e.data) {
				field_input(clchars(e.data));
				hiddenInput.value = ''; // 清空输入框
			}
		});
...
// 激活输入法支持
function activateInputMethod() {
	if (wid.infield && window.chineseInputEnabled) {
		const input = createHiddenInput();
		input.focus();
		// 移动端需要延迟聚焦
		setTimeout(() => {
			if (wid.infield && window.chineseInputEnabled) input.focus();
		}, 100);
	}
}
...
q('body').onkeydown=e=>{
	// 如果焦点在 Spectrum 输入框中，不处理 Decker 的键盘事件
	const activeElement = document.activeElement;
	if(activeElement && (activeElement.classList.contains('sp-input') || activeElement.closest('.sp-container'))) {
		return; // 让 Spectrum 处理键盘事件
	}
	...
	// 修改输入处理逻辑：对于输入框，根据中文输入开关决定处理方式
	else if(e.key.length==1&&wid.infield){
		// 如果启用了中文输入模式
		if(window.chineseInputEnabled) {
			// 检查是否是输入法相关按键或输入法正在激活状态
			if(e.key === 'Process' || e.isComposing || isInputMethodActive) {
				// 输入法正在工作，激活隐藏输入框，阻止默认行为
				activateInputMethod();
				e.preventDefault();
				return;
			}
			// 对于中文输入模式下的普通字符，也需要通过隐藏输入框处理
			// 以确保能正确触发输入法
			activateInputMethod();
			e.preventDefault();
			return;
...
dopaste=(x,pos)=>{
	...
	if((ms.type==null||(ms.type=='contraption_props'&&wid.fv))&&/^%%IMG[012]/.test(x)){
		...
		}else{setmode('draw'),bg_paste(i,0,pos)}
	}
...
// 确保颜色选择器在屏幕内的通用函数
// 计算颜色选择器的居中位置（用于预先设置位置）
const calculateColorPickerCenterPosition = (containerWidth, containerHeight) => {
	// 检测是否在iframe中
	const isInIframe = window.self !== window.top;
	let screenWidth, screenHeight;
	let iframeOffsetX = 0, iframeOffsetY = 0;
	let iframeVisibleWidth = 0, iframeVisibleHeight = 0;
	...
	if(isInIframe) {
		try {
			...
			const parentViewportWidth = parentWindow.innerWidth || parentWindow.document.documentElement.clientWidth;
			const parentViewportHeight = parentWindow.innerHeight || parentWindow.document.documentElement.clientHeight;
			canAccessParent = true;
			
			// 获取iframe在父窗口中的位置和可见区域
			try {
				...
				if(iframe && iframe.getBoundingClientRect) {
					...
					// 计算iframe在父窗口中的可见区域（考虑裁剪）
					const iframeLeft = iframeRect.left;
					const iframeTop = iframeRect.top;
					const iframeRight = iframeRect.right;
					const iframeBottom = iframeRect.bottom;
					
					// 计算可见区域的边界（与父窗口视口的交集）
					const visibleLeft = Math.max(0, iframeLeft);
					const visibleTop = Math.max(0, iframeTop);
					const visibleRight = Math.min(parentViewportWidth, iframeRight);
					const visibleBottom = Math.min(parentViewportHeight, iframeBottom);
					
					// 计算iframe的可见宽度和高度
					iframeVisibleWidth = Math.max(0, visibleRight - visibleLeft);
					iframeVisibleHeight = Math.max(0, visibleBottom - visibleTop);
					
					// 使用iframe的可见区域作为屏幕尺寸
					screenWidth = parentViewportWidth;
					screenHeight = parentViewportHeight;
					
					// 如果iframe完全不可见，使用iframe自身的尺寸作为后备
					if(iframeVisibleWidth === 0 || iframeVisibleHeight === 0) {
						iframeVisibleWidth = window.innerWidth || document.documentElement.clientWidth;
						iframeVisibleHeight = window.innerHeight || document.documentElement.clientHeight;
					}
				} else {
					// 无法获取iframe元素，使用父窗口尺寸
					screenWidth = parentViewportWidth;
					screenHeight = parentViewportHeight;
					iframeVisibleWidth = window.innerWidth || document.documentElement.clientWidth;
					iframeVisibleHeight = window.innerHeight || document.documentElement.clientHeight;
			...
			// 跨域限制，无法访问父窗口，使用iframe自身的尺寸
			screenWidth = window.innerWidth || document.documentElement.clientWidth;
			screenHeight = window.innerHeight || document.documentElement.clientHeight;
			iframeVisibleWidth = screenWidth;
			iframeVisibleHeight = screenHeight;
	...
	// 获取iframe内部的尺寸（无论是否在iframe中）
	const iframeInternalWidth = window.innerWidth || document.documentElement.clientWidth;
	const iframeInternalHeight = window.innerHeight || document.documentElement.clientHeight;
	
	let leftPos, topPos;
	
	// 如果在iframe中，优先在iframe内部居中显示（这样可以避免旋转问题）
	if(isInIframe) {
		// 在iframe内部居中
		const centerX = iframeInternalWidth / 2;
		const centerY = iframeInternalHeight / 2;
		
		// 计算居中位置
		leftPos = centerX - containerWidth / 2;
		topPos = centerY - containerHeight / 2;
		
		// 确保在iframe内部边界内
		leftPos = Math.max(10, Math.min(leftPos, iframeInternalWidth - containerWidth - 10));
		topPos = Math.max(10, Math.min(topPos, iframeInternalHeight - containerHeight - 10));
	} else {
		// 不在iframe中，在屏幕中心显示
		...
		// 确保在屏幕边界内
		leftPos = Math.max(10, Math.min(leftPos, screenWidth - containerWidth - 10));
		topPos = Math.max(10, Math.min(topPos, screenHeight - containerHeight - 10));
	}
	
	// 如果在iframe中且可以访问父窗口，检查是否在父窗口的可见区域内
	if(isInIframe && canAccessParent) {
		// 计算iframe在父窗口中的可见区域边界
		const iframeVisibleLeft = Math.max(0, iframeOffsetX);
		const iframeVisibleTop = Math.max(0, iframeOffsetY);
		const iframeVisibleRight = Math.min(screenWidth, iframeOffsetX + iframeVisibleWidth);
		const iframeVisibleBottom = Math.min(screenHeight, iframeOffsetY + iframeVisibleHeight);
		
		// 计算颜色选择器在父窗口中的实际位置
		const actualLeft = iframeOffsetX + leftPos;
		const actualTop = iframeOffsetY + topPos;
		const actualRight = actualLeft + containerWidth;
		const actualBottom = actualTop + containerHeight;
		
		// 如果超出可见区域，调整位置（但保持在iframe内部）
		if(actualRight > iframeVisibleRight) {
			// 向左调整，但确保不超出iframe左边界
			const maxLeft = iframeVisibleRight - containerWidth - iframeOffsetX - 10;
			leftPos = Math.max(10, Math.min(maxLeft, leftPos));
		}
		if(actualBottom > iframeVisibleBottom) {
			// 向上调整，但确保不超出iframe上边界
			const maxTop = iframeVisibleBottom - containerHeight - iframeOffsetY - 10;
			topPos = Math.max(10, Math.min(maxTop, topPos));
		}
		if(actualLeft < iframeVisibleLeft) {
			// 向右调整，但确保不超出iframe右边界
			const minLeft = iframeVisibleLeft - iframeOffsetX + 10;
			leftPos = Math.min(iframeInternalWidth - containerWidth - 10, Math.max(minLeft, leftPos));
		}
		if(actualTop < iframeVisibleTop) {
			// 向下调整，但确保不超出iframe下边界
			const minTop = iframeVisibleTop - iframeOffsetY + 10;
			topPos = Math.min(iframeInternalHeight - containerHeight - 10, Math.max(minTop, topPos));
		}
	} else if(!isInIframe) {
		// 不在iframe中，如果容器比屏幕还大，至少确保左上角在屏幕内
...
const ensureColorPickerInView = () => {
	const spContainer = q('.sp-container');
	if(!spContainer) return;
	
	spContainer.style.position = 'fixed';
	spContainer.style.transform = 'none';
	spContainer.style.zIndex = '10001';
	
	// 获取调色板尺寸
	const containerRect = spContainer.getBoundingClientRect();
	const containerWidth = containerRect.width;
	const containerHeight = containerRect.height;
	
	// 使用计算函数获取居中位置
	const position = calculateColorPickerCenterPosition(containerWidth, containerHeight);
	spContainer.style.top = position.top + 'px';
	spContainer.style.left = position.left + 'px';
};

// 监听窗口大小改变，确保颜色选择器始终在屏幕内
...
window.addEventListener('resize', () => {
	...
	resizeTimeout = setTimeout(() => {
		...
		if(spContainer && spContainer.style.display !== 'none') {
			ensureColorPickerInView();
		}
	}, 30);
});

// 调色板功能实现
show_color_picker_for_pattern=patternIndex=>{
	console.log('为图案索引显示调色板:', patternIndex)
	
	// 检查deck.patterns是否存在
	if(!deck.patterns) {
		console.error('deck.patterns不存在，无法编辑颜色')
		return
	}
	
	// 获取当前图案的颜色值
	const currentColorValue = ln(dget(deck.patterns, lmn(patternIndex))) || 0
	const currentColor = '#' + ('000000' + currentColorValue.toString(16)).slice(-6)
	
	// 显示调色板模态框（但设置为透明，只显示Spectrum调色板）
	const modal = q('#colorPickerModal')
	modal.style.display = 'block'
	// 隐藏模态框的背景和边框，只保留输入框供Spectrum使用
	modal.style.background = 'transparent';
	modal.style.border = 'none';
	modal.style.padding = '0';
	// 预先计算居中位置（使用估算的调色板尺寸：300x400）
	const estimatedPosition = calculateColorPickerCenterPosition(300, 400);
	modal.style.top = estimatedPosition.top + 'px';
	modal.style.left = estimatedPosition.left + 'px';
	modal.style.transform = 'none';
	// 隐藏标题和按钮
	const modalTitle = modal.querySelector('div[style*="font-weight:bold"]');
	const modalButtons = modal.querySelector('div[style*="margin-top:10px"]');
	if(modalTitle) modalTitle.style.display = 'none';
	if(modalButtons) modalButtons.style.display = 'none';
	
	// 禁用底层canvas的点击事件
	const disableCanvasEvents = () => {
		const displayCanvas = q('#display');
		const ltoolsCanvas = q('#ltools');
		const rtoolsCanvas = q('#rtools');
		if(displayCanvas) {
			displayCanvas.style.pointerEvents = 'none';
		}
		if(ltoolsCanvas) {
			ltoolsCanvas.style.pointerEvents = 'none';
		}
		if(rtoolsCanvas) {
			rtoolsCanvas.style.pointerEvents = 'none';
		}
	};
	...
	const enableCanvasEvents = () => {
		...
		if(ltoolsCanvas) {
			ltoolsCanvas.style.pointerEvents = '';
		}
		if(rtoolsCanvas) {
			rtoolsCanvas.style.pointerEvents = '';
		}
	};
	...
	if(typeof $ !== 'undefined' && $.fn.spectrum) {
		// 确保输入框类型是text（Spectrum需要text类型）
		const colorInput = q('#colorPicker1');
		colorInput.type = 'text';
		$('#colorPicker1').spectrum('destroy') // 先销毁之前的实例
		$('#colorPicker1').spectrum({
			...
			preferredFormat: 'hex',
			showAlpha: false,
			chooseText: _t('choose'),
			cancelText: _t('cancel'),
			togglePaletteMoreText: _t('more'),
			togglePaletteLessText: _t('less'),
			clearText: _t('clear'),
			noColorSelectedText: _t('no_color_selected'),
			hide: function(color) {
				// 当调色板隐藏时，也隐藏外部模态框（如果还未关闭）
				if(modal.style.display !== 'none') {
					...
					const modalTitle8 = modal.querySelector('div[style*="font-weight:bold"]');
					const modalButtons8 = modal.querySelector('div[style*="margin-top:10px"]');
					const colorInput8 = q('#colorPicker1');
					if(modalTitle8) modalTitle8.style.display = '';
					if(modalButtons8) modalButtons8.style.display = '';
					if(colorInput8) colorInput8.style.display = '';
					enableCanvasEvents();
				}
			},
			cancel: function() {
				...
				modal.style.left = '';
				modal.style.transform = '';
				const modalTitle9 = modal.querySelector('div[style*="font-weight:bold"]');
				const modalButtons9 = modal.querySelector('div[style*="margin-top:10px"]');
				const colorInput9 = q('#colorPicker1');
				if(modalTitle9) modalTitle9.style.display = '';
				if(modalButtons9) modalButtons9.style.display = '';
				if(colorInput9) colorInput9.style.display = '';
				enableCanvasEvents();
			}
		})
		...
		// 自动显示颜色选择器，无需再点击输入框
		setTimeout(() => {
			$('#colorPicker1').spectrum('show');
			// 立即调整 Spectrum 调色板的位置到中间，确保在屏幕范围内
			requestAnimationFrame(() => {
				ensureColorPickerInView();
				// 隐藏 .sp-original-input-container 元素
				const originalInputContainer = document.querySelector('.sp-original-input-container');
		...
		// 如果没有Spectrum.js，使用原生颜色选择器
		const colorInput = q('#colorPicker1');
		// 将输入框类型改为color以使用原生颜色选择器
		...
		if(modalButtons) modalButtons.style.display = '';
		// 阻止颜色选择器相关元素的点击事件冒泡，防止意外关闭模态框
		...
		// 注意：不需要在按钮上添加 stopPropagation()
		// 因为模态框的点击处理器检查 e.target === modal，按钮点击时不会关闭模态框
		// 添加颜色变化监听
	...
	q('#colorOk').onclick = _=> {
		...
		if(typeof $ !== 'undefined' && $.fn.spectrum) {
			selectedColor = $('#colorPicker1').spectrum('get').toHexString()
		} else {
			selectedColor = q('#colorPicker1').value
		}
		
		console.log('选择的颜色:', selectedColor, '应用到图案索引:', patternIndex)
		
		// 检查deck.patterns是否存在
		if(!deck.patterns) {
			console.error('deck.patterns不存在，无法应用颜色')
			modal.style.display = 'none'
			enableCanvasEvents();
			return
		}
		...
		iwrite(deck.patterns, lmn(patternIndex), lmn(colorValue))
		...
		modal.style.left = '';
		modal.style.transform = '';
		const modalTitle2 = modal.querySelector('div[style*="font-weight:bold"]');
		const modalButtons2 = modal.querySelector('div[style*="margin-top:10px"]');
		const colorInput2 = q('#colorPicker1');
		if(modalTitle2) modalTitle2.style.display = '';
		if(modalButtons2) modalButtons2.style.display = '';
		if(colorInput2) colorInput2.style.display = '';
		enableCanvasEvents();
		mark_dirty()
	}
	
	// 取消按钮事件
	q('#colorCancel').onclick = _=> {
		modal.style.display = 'none'
		// 恢复模态框的默认样式（供下次使用）
		modal.style.background = '';
		modal.style.border = '';
		...
		modal.style.left = '';
		modal.style.transform = '';
		const modalTitle3 = modal.querySelector('div[style*="font-weight:bold"]');
		const modalButtons3 = modal.querySelector('div[style*="margin-top:10px"]');
		const colorInput3 = q('#colorPicker1');
		if(modalTitle3) modalTitle3.style.display = '';
		if(modalButtons3) modalButtons3.style.display = '';
		if(colorInput3) colorInput3.style.display = '';
		enableCanvasEvents();
	}
	
	// 点击模态框外部关闭（与 show_color_picker 使用相同的逻辑）
	modal.onclick = e => {
		// 只允许点击模态框本身（背景）时关闭，不允许点击子元素时关闭
		if(e.target === modal) {
			const colorInput5 = q('#colorPicker1');
			// 如果使用原生颜色选择器，禁用点击外部关闭功能
			// 因为原生颜色选择器打开时，用户可能需要点击周围区域
			if(colorInput5 && colorInput5.type === 'color') {
				// 原生颜色选择器模式下，不自动关闭，用户需要通过按钮关闭
				return;
			}
			// Spectrum模式下，保持原有行为
			modal.style.display = 'none';
			...
			modal.style.left = '';
			modal.style.transform = '';
			const modalTitle5 = modal.querySelector('div[style*="font-weight:bold"]');
			const modalButtons5 = modal.querySelector('div[style*="margin-top:10px"]');
			if(modalTitle5) modalTitle5.style.display = '';
			if(modalButtons5) modalButtons5.style.display = '';
			if(colorInput5) colorInput5.style.display = '';
			enableCanvasEvents();
		}
	};
}
...
show_color_picker=_=>{
	console.log('调色板被调用，当前wid状态:', wid)
	...
	const savedContext = {
		...
		fv: wid.fv,
		infield: wid.infield,
		cursor: wid.cursor ? {...wid.cursor} : null
	};
	console.log('保存的上下文:', savedContext);
	
	// 获取当前文本框的文本内容
	let selectedText = '';
	try {
		// 如果当前有文本编辑状态，获取文本内容
		if(wid.fv && wid.fv.table) {
			selectedText = wid.fv.table.v.get('text')[0].v;
		}
	} catch(e) {
		console.log('获取文本内容失败:', e);
		selectedText = '';
	}
	...
	if(modalTitle) modalTitle.style.display = 'none';
	if(modalButtons) modalButtons.style.display = 'none';
	if(colorInput) colorInput.style.display = 'none'; // 隐藏输入框，只显示 Spectrum 调色板
	...
	// 设置初始颜色值，确保格式正确（添加#前缀如果缺失）
	let currentColor = selectedText.trim() || '#ff0000';
	// 如果颜色值没有#前缀，添加它
	if(currentColor.length === 6 && /^[0-9A-Fa-f]{6}$/.test(currentColor)) {
		currentColor = '#' + currentColor;
	} else if(currentColor.length !== 7 || !currentColor.startsWith('#')) {
		// 如果格式不正确，使用默认值
		currentColor = '#ff0000';
	}
	...
	const applyColorAndClose = () => {
		console.log('自动应用颜色:', currentColor);
		if(currentColor.length!=7) return;
		
		window.currentColor2=currentColor.slice(1,7);
		...
		// 方式2：直接操作保存的文本字段
		setTimeout(() => {
			if(!applied && savedContext.fv && savedContext.fv.table) {
			try {
				savedContext.fv.table.v.set('text', [{t: 'str', v: window.currentColor2}]);
				field_notify();
				console.log('方式2成功：直接设置文本字段为:', window.currentColor2);
		...
		if(modalTitle) modalTitle.style.display = '';
		if(modalButtons) modalButtons.style.display = '';
		if(colorInput) colorInput.style.display = '';
		enableCanvasEvents();
	};
	
	// 初始化Spectrum颜色选择器（禁用动画）
	if(typeof $ !== 'undefined' && $.fn.spectrum) {
		// 确保输入框类型是text（Spectrum需要text类型）
		const colorInput = q('#colorPicker1');
		colorInput.type = 'text';
		...
		$('#colorPicker1').spectrum({
			...
			showInput: true,
			// showPalette: true,
			showSelectionPalette: false, // 禁用选择调色板避免动画
			animationSpeed: 0, // 禁用动画
			// palette: [
			...
			hide: function(color) {
				...
				if(modal.style.display !== 'none') {
					...
					modal.style.left = '';
					modal.style.transform = '';
					const modalTitle6 = modal.querySelector('div[style*="font-weight:bold"]');
					const modalButtons6 = modal.querySelector('div[style*="margin-top:10px"]');
					const colorInput6 = q('#colorPicker1');
					if(modalTitle6) modalTitle6.style.display = '';
					if(modalButtons6) modalButtons6.style.display = '';
					if(colorInput6) colorInput6.style.display = '';
					enableCanvasEvents();
				}
			},
			cancel: function() {
				...
				modal.style.left = '';
				modal.style.transform = '';
				const modalTitle7 = modal.querySelector('div[style*="font-weight:bold"]');
				const modalButtons7 = modal.querySelector('div[style*="margin-top:10px"]');
				const colorInput7 = q('#colorPicker1');
				if(modalTitle7) modalTitle7.style.display = '';
				if(modalButtons7) modalButtons7.style.display = '';
				if(colorInput7) colorInput7.style.display = '';
				enableCanvasEvents();
			},
			showAlpha:false,
			// 添加中文本地化设置
	...
	// 确定按钮事件（保留作为手动确认的备用方式，但通常不需要，因为颜色改变时已自动应用）
	q('#colorOk').onclick = _=> {
		applyColorAndClose();
	};
	...
	q('#colorCancel').onclick = _=> {
		...
		modal.style.left = '';
		modal.style.transform = '';
		const modalTitle4 = modal.querySelector('div[style*="font-weight:bold"]');
		const modalButtons4 = modal.querySelector('div[style*="margin-top:10px"]');
		const colorInput4 = q('#colorPicker1');
		if(modalTitle4) modalTitle4.style.display = '';
		if(modalButtons4) modalButtons4.style.display = '';
		if(colorInput4) colorInput4.style.display = '';
		enableCanvasEvents();
	};
	
	// 点击模态框外部关闭
	// 注意：由于已经为颜色选择器、标题、按钮等元素添加了 stopPropagation()，
	// 所以只有当真正点击模态框背景时，e.target 才会是 modal
	modal.onclick = e => {
		// 只允许点击模态框本身（背景）时关闭，不允许点击子元素时关闭
		if(e.target === modal) {
			const colorInput5 = q('#colorPicker1');
...
// 应用颜色到文本的函数
apply_color_to_text=(color, selectedText, hasSelection)=>{
	if(!wid.fv) return;
	...
	if(hasSelection && selectedText.trim()) {
		// 尝试将选中文本解析为颜色值
		const textColor = selectedText.trim();
		// 检查是否为有效的颜色格式（十六进制、rgb、颜色名称等）
		if(/^#[0-9A-Fa-f]{3,6}$/.test(textColor) || 
		   /^rgb\(\s*\d+\s*,\s*\d+\s*,\s*\d+\s*\)$/i.test(textColor) ||
		   /^rgba\(\s*\d+\s*,\s*\d+\s*,\s*\d+\s*,\s*[0-9.]+\s*\)$/i.test(textColor) ||
		   /^(red|blue|green|yellow|black|white|gray|orange|purple|pink|brown)$/i.test(textColor)) {
			processedColor = textColor;
		}
	}
	...
	// 插入处理后的颜色值
	field_input(processedColor);
};

pushstate(lmenv()),load_deck(deck_read(q('script[language="decker"]')?.innerText || window.DECK_DATA))
window.DECK_DATA=''
const tag=decodeURI(document.URL.split('#')[1]||'');if(tag.length)iwrite(deck,lms('card'),lms(tag))
resize(),requestAnimationFrame(loop)

// 添加全局函数到 window 对象，用于加载/导入 .deck 文件
window.loadDeckFile = function(callback) {
	if (typeof callback !== 'function') {
		console.error('loadDeckFile: callback must be a function');
		return;
	}
	
	open_text('.deck,.html', function(text) {
		try {
			const deckData = deck_read(text);
			callback(null, deckData, text);
		} catch (error) {
...
// 添加异步版本的函数，返回 Promise
window.loadDeckFileAsync = function() {
	return new Promise((resolve, reject) => {
		window.loadDeckFile((error, deckData, rawText) => {
			if (error) {
				reject(error);
			} else {
				resolve({ deckData, rawText });
			}
		});
	});
};

// 添加全局函数到 window 对象，用于导出当前 deck
window.exportDeckFile = function(filename, format) {
	if (!filename) {
		filename = 'exported_deck.deck';
	}
...
// 添加函数获取当前 deck 的序列化数据（不触发下载）
window.getDeckData = function(format) {
	try {
		const serialized = deck_write(deck);
		isSaved = true;
		if (format === 'html') {
			// 如果需要 HTML 格式，使用 save_deck 的逻辑
			return deck_html(deck, serialized);
		} else {
			return serialized;
		}
	} catch (error) {
		console.error('Error serializing deck:', error);
		return null;
	}

};

// 添加函数获取当前画布的 GIF 数据（不触发下载）
// 共同的 GIF 生成逻辑（与 save_all_wiggler_gif 一致）
const generateGifFromFrames = (frames, delays, transparent, options) => {
	if (!frames || frames.length === 0) return null;
	
	// 使用与 save_all_wiggler_gif 相同的 writegif 调用方式
	const gifData = writegif(frames, delays, transparent);
	
	// 根据选项返回不同格式
	if (options && options.format === 'base64') {
		// 返回 Base64 字符串
		const base64 = btoa(String.fromCharCode.apply(null, Uint8Array.from(gifData)));
		return options.dataUrl ? `data:image/gif;base64,${base64}` : base64;
	} else if (options && options.format === 'blob') {
		// 返回 Blob 对象
		return new Blob([Uint8Array.from(gifData)], { type: 'image/gif' });
	} else {
		// 默认返回 Uint8Array（与 save_all_wiggler_gif 的 writegif 返回格式一致）
		return new Uint8Array(gifData);
	}
};

window.getGifData = function(options) {
	try {
		// 默认选项
		options = options || {};
		const transparent = options.transparent !== false; // 默认透明
		const delay = options.delay || 10; // 默认延迟 10（0.1秒）
		
		// 检查必要的全局变量
		if (!deck || !dr || typeof con === 'undefined') {
			console.error('Required context not available');
			return null;
		}
		
		// 处理选择区域（如果有）
		if (bg_has_sel()) {
			const s = rcopy(dr.sel_here);
			bg_end_selection();
			dr.sel_here = s;
		}
		
		// 获取当前画布图像
		let i = draw_con(con(), 1);
		let off = rect();
		let f = 1;
		let a = 0;
		let bg = dr.trans ? 0 : 32;
		let frames = [];
		...
		// 动画模式处理函数
		const anim_pattern = (pix, x, y, f) => {
			return pix < 28 || pix > 31 ? pix : anim[pix - 28][f % max(1, anim[pix - 28].length)];
		};
		
		// 图案绘制函数
		const draw_pattern = (pix, x, y) => {
			return pix < 2 ? (pix ? 1 : 0) : 
			       pix > 31 ? (pix == 32 ? 0 : 1) : 
			       pal[(x % 8) + (8 * (y % 8)) + (8 * 8 * pix)] & 1;
		};
		
		// 处理选择区域或套索
		if (bg_has_sel() || bg_has_lasso()) {
			const r = rclip(dr.sel_here, con_dim());
			i = image_copy(i, r);
			off = rcopy(r);
		}
		
		// 计算动画帧数
		for (let z = 0; z < 4 && dr.show_anim; z++) {
			const c = anim[z].length;
			if (c) f = lcm(f, c);
		}
		...
		for (let z = 0; z < f; z++) {
			const frame = image_copy(i);
			for (let y = 0; y < i.size.y; y++) {
				for (let x = 0; x < i.size.x; x++) {
					const v = frame.pix[x + (i.size.x * y)];
					if (v >= 28 && v <= 31) a = 1; // 检测到动画像素
					const c = anim_pattern(v, x, y, z);
					const p = draw_pattern(c, x + off.x, y + off.y);
					frame.pix[x + (i.size.x * y)] = c >= 32 ? c : (p ? 1 : bg);
				}
			}
			frames.push(bg_has_lasso() ? image_mask(frame, dr.mask) : frame);
		...
		// 准备 frames 和 delays（与 save_all_wiggler_gif 的格式一致）
		const finalFrames = a ? frames : [frames[0]];
		const finalDelays = a ? frames.map(() => delay) : [delay];
		
		// 使用共同的 GIF 生成逻辑
		return generateGifFromFrames(finalFrames, finalDelays, transparent, options);
	} catch (error) {
...
let autoSwitchState = {
	enabled: false,
	startCard: 1,
	endCard: Math.max(1, getCardCount() - 5),
	currentCard: 1,
	intervals: [], // 每张卡片的间隔时间（毫秒）
	uniformInterval: 100, // 统一间隔时间（毫秒）
	useUniformInterval: true,
	timer: null,
	startTime: 0,
	multiSelect: false, // 多选模式
	selectedCards: [] // 选中的卡片索引数组
};

// 复制并粘贴卡片功能
function copyAndPasteCard() {
	// 直接复制当前卡片，不使用剪贴板
	const currentCard = ifield(deck, 'card')
	if(currentCard) {
		// 使用deck_copy和deck_paste函数正确复制卡片
		const copiedCardData = deck_copy(deck, currentCard)
		const copiedCard = deck_paste(deck, copiedCardData)
		con_set(null)
		// 设置新卡片的索引
		const n = ln(ifield(currentCard, 'index'))
		iwrite(copiedCard, lms('index'), lmn(n + 1))
		// 切换到新卡片
		n_go([copiedCard], deck)
		mark_dirty()
	}
}
...
function startAutoSwitch() {
	...
	autoSwitchState.enabled = true;
	autoSwitchState.currentCard = autoSwitchState.startCard;
	autoSwitchState.startTime = Date.now();
	
	// 导航到起始卡片
	n_go([lmn(autoSwitchState.currentCard)], deck);
...
function stopAutoSwitch() {
	...
	autoSwitchState.enabled = false;
	if (autoSwitchState.timer) {
		clearTimeout(autoSwitchState.timer);
		autoSwitchState.timer = null;
	}
}
...
function scheduleNextSwitch() {
	...
	let interval;
	const cardIndex = autoSwitchState.currentCard - autoSwitchState.startCard;
	// 确保间隔数组有足够的元素
	while(autoSwitchState.intervals.length <= cardIndex) {
		autoSwitchState.intervals.push(autoSwitchState.uniformInterval)
	}
	interval = autoSwitchState.intervals[cardIndex] || autoSwitchState.uniformInterval;
	
	autoSwitchState.timer = setTimeout(() => {
		// 切换到下一张卡片
		autoSwitchState.currentCard++;
		
		// 检查是否超出范围
		if (autoSwitchState.currentCard > autoSwitchState.endCard) {
			autoSwitchState.currentCard = autoSwitchState.startCard; // 循环回到起始卡片
		}
...
function pasteCard() {
	getclipboard(text => {
		if (text && /^%%CRD0/.test(text)) {
			const c = deck_paste(deck, lms(text));
			con_set(null);
			const card = ifield(deck, 'card');
			const n = ln(ifield(card, 'index'));
			iwrite(c, lms('index'), lmn(n + 1));
			n_go([c], deck);
			mark_dirty();
		}
	});
...
function getCardCount() {
	return count(ifield(deck, 'cards'));
}

// 获取当前卡片索引
function getCurrentCardIndex() {
	return ln(ifield(ifield(deck, 'card'), 'index'));
}
...
function startCanvasRecording() {
	...
	if (!recordingCanvas) {
		...
		recordingContext.webkitImageSmoothingEnabled = false;
		recordingContext.mozImageSmoothingEnabled = false;
		recordingContext.msImageSmoothingEnabled = false;
	}
	
	// 设置Canvas尺寸为当前deck尺寸
	const deckSize = deck.size;
	recordingCanvas.width = deckSize.x;
	recordingCanvas.height = deckSize.y;
	
	// 获取Canvas的MediaStream
	try {
		recordingStream = recordingCanvas.captureStream(recordingFrameRate);
		
		// 创建MediaRecorder，使用更高质量设置
		recordedChunks = [];
		let recorderOptions;
		...
		if (MediaRecorder.isTypeSupported('video/webm;codecs=vp9')) {
			recorderOptions = {
				...
				videoBitsPerSecond: 8000000 // 8Mbps 高质量
			};
		...
		canvasRecorder = new MediaRecorder(recordingStream, recorderOptions);
...
// 为录制启动自动播放
function startAutoSwitchForRecording() {
	console.log('开始自动播放用于录制...');
	...
	// 计算总播放时间
	const totalCards = autoSwitchState.endCard - autoSwitchState.startCard + 1;
	let totalTime = 0;
	for (let i = 0; i < totalCards; i++) {
		const interval = autoSwitchState.intervals[i] || autoSwitchState.uniformInterval;
		totalTime += interval;
	}
	
	console.log(`预计录制时间: ${totalTime}ms`);
	
	// 设置录制结束定时器
	setTimeout(() => {
		stopCanvasRecording();
	}, totalTime + 1000); // 额外1秒缓冲时间
}

// 停止Canvas录制
function stopCanvasRecording() {
	console.log('停止Canvas录制...');
...
function saveRecordedVideo() {
	...
	try {
		...
		a.href = url;
		a.download = `deck_recording_${new Date().getTime()}.webm`;
...
function updateRecordingCanvas() {
	if (!isRecording || !recordingCanvas || !recordingContext) {
		return;
	}
	
	try {
		// 获取当前渲染的Canvas内容
		const renderCanvas = q('#render');
		if (!renderCanvas) {
			return;
		}
		...
		// 将渲染Canvas的内容精确复制到录制Canvas（1:1像素映射）
		recordingContext.drawImage(renderCanvas, 0, 0);
		
	} catch (error) {
		console.error('更新录制Canvas失败:', error);
	}
}

// 不在浏览器中执行
if (window.isInWebView) {
	// 固定的原始 canvas 尺寸（decker 默认尺寸）
	const CANVAS_ORIGINAL_WIDTH = 512;
	const CANVAS_ORIGINAL_HEIGHT = 342;
	...
	window.canvasTransform = {
		...
		offsetX: 0,
		offsetY: 0,
		originalWidth: CANVAS_ORIGINAL_WIDTH,
		originalHeight: CANVAS_ORIGINAL_HEIGHT,
		isRotated: false  // 是否旋转了90度
	};
	
	// 防止循环调用的标志
	let isAdjusting = false;
	
	// 重写 decker 的 resize 函数，在 WebView 中让 zoom 始终为 1
	// 这样 canvas 像素尺寸保持原始的 512 x 342，缩放完全由 CSS transform 处理
	const originalDeckerResize = window.resize;
	window.resize = function() {
		// 强制 zoom 为 1
		window.zoom = 1;
		window.tzoom = 0;  // 禁用工具栏缩放
		
		const c = document.getElementById('display');
		const r = document.getElementById('render');
		const tl = document.getElementById('ltools');
		const tr = document.getElementById('rtools');
		const rl = document.getElementById('lrender');
		const rr = document.getElementById('rrender');
		
		// 设置 canvas 为固定的原始尺寸
		if (c) { c.width = CANVAS_ORIGINAL_WIDTH; c.height = CANVAS_ORIGINAL_HEIGHT; }
		if (r) { r.width = CANVAS_ORIGINAL_WIDTH; r.height = CANVAS_ORIGINAL_HEIGHT; }
		if (tl) { tl.width = 0; tl.height = 0; }
		if (tr) { tr.width = 0; tr.height = 0; }
		if (rl) { rl.width = 0; rl.height = 0; }
		if (rr) { rr.width = 0; rr.height = 0; }
		
		console.log('WebView resize: zoom=1, canvas=' + CANVAS_ORIGINAL_WIDTH + 'x' + CANVAS_ORIGINAL_HEIGHT);
		
		// 在 decker resize 后重新调整 CSS transform
		setTimeout(adjustCanvasSizeForMobile, 50);
	};

	// 动态调整 canvas 显示以适配 WebView，按比例缩放
	// 当高度大于宽度时（竖屏），旋转90度
	// 使用纯 CSS transform 来处理缩放和旋转
	function adjustCanvasSizeForMobile() {
		...
		// 使用固定的原始尺寸（因为我们重写了 resize，zoom 始终为 1）
		const canvasWidth = CANVAS_ORIGINAL_WIDTH;
		const canvasHeight = CANVAS_ORIGINAL_HEIGHT;
		...
		if (isPortrait) {
			// 竖屏模式：旋转90度后，canvas的宽高对调来计算缩放
			// 旋转后，原来的宽变成高，原来的高变成宽
			const scaleX = viewportWidth / canvasHeight;   // 视口宽度 / canvas高度（旋转后变成宽）
			const scaleY = viewportHeight / canvasWidth;   // 视口高度 / canvas宽度（旋转后变成高）
			scale = Math.min(scaleX, scaleY);
			
			// 旋转后的视觉尺寸
			const visualWidth = canvasHeight * scale;   // 旋转后宽 = 原高 * scale
			const visualHeight = canvasWidth * scale;   // 旋转后高 = 原宽 * scale
			offsetX = (viewportWidth - visualWidth) / 2;
			offsetY = (viewportHeight - visualHeight) / 2;
			
			console.log('Portrait mode - Scale:', scale.toFixed(3));
			console.log('Visual size after rotation:', visualWidth.toFixed(1), 'x', visualHeight.toFixed(1));
			
			// 让元素中心在视口中心，然后围绕中心旋转和缩放
			// 元素中心 = left + canvasWidth/2, top + canvasHeight/2
			// 要让中心在视口中心：left = viewportWidth/2 - canvasWidth/2
			const left = viewportWidth / 2 - canvasWidth / 2;
			const top = viewportHeight / 2 - canvasHeight / 2;
			...
			// 只做旋转和缩放，不需要 translate（因为已经通过 left/top 居中了）
			display.style.transform = `rotate(90deg) scale(${scale})`;
			...
			console.log('Landscape mode - Scale:', scale.toFixed(3));
		...
		// 确保 body 和 html 不滚动，并移除可能影响定位的样式
		document.body.style.overflow = 'hidden';
		document.body.style.display = 'block';  // 覆盖 flex 布局
	...
	// 重写mouse函数以正确处理坐标转换（包括旋转）
	// 使用 CSS transform: translate(-50%, -50%) rotate() scale() 后的坐标转换
	if (typeof window.originalMouse === 'undefined') {
		// 保存原始的mouse函数
		window.originalMouse = window.mouse;
		
		// 重写mouse函数
		window.mouse = function(e, f) {
			const transform = window.canvasTransform;
			let originalX, originalY;
			
			// canvas 中心在视口中心
			const centerX = transform.viewportWidth / 2;
			const centerY = transform.viewportHeight / 2;
			
			// 触摸点相对于视口中心的偏移
			const touchX = e.pageX - centerX;
			const touchY = e.pageY - centerY;
			
			if (transform.isRotated) {
				// 竖屏旋转模式：需要逆向旋转坐标
				// CSS transform 是 rotate(90deg)，所以逆向是 rotate(-90deg)
				// 逆向旋转 -90 度：x' = y, y' = -x
				const rotatedX = touchY;
				const rotatedY = -touchX;
				
				// 除以缩放比例，转换到 canvas 坐标系
				originalX = (rotatedX / transform.scale) + (transform.originalWidth / 2);
				originalY = (rotatedY / transform.scale) + (transform.originalHeight / 2);
				
			} else {
				// 横屏正常模式：只需要处理缩放
				// 除以缩放比例，加上 canvas 中心偏移
				originalX = (touchX / transform.scale) + (transform.originalWidth / 2);
				originalY = (touchY / transform.scale) + (transform.originalHeight / 2);
			}
			
			// 设置原始位置信息
			if (typeof ev !== 'undefined') {
				ev.rawpos = {x: e.pageX, y: e.pageY};
			}
			
			// 检查中键
			if (e.button == 1) {
				if (typeof down !== 'undefined' && f == down && typeof ev !== 'undefined') ev.mdown = 1;
				return;
			}
			
			// 调用处理函数，传入转换后的坐标
			f(Math.floor(originalX), Math.floor(originalY), e.button != 0);
		};
	}

	// 页面加载完成后调整尺寸
	document.addEventListener('DOMContentLoaded', function() {
		setTimeout(adjustCanvasSizeForMobile, 100);
	});
...
window.addEventListener('beforeunload', function(e) {
	if (!isSaved) {
		e.preventDefault()
		e.returnValue = _t('unsaved_changes_leave')
		return _t('unsaved_changes_leave')
	}
})
